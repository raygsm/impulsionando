...
const migration = readFileSync(
  "supabase/migrations/20260807212613_chrismed_events_registration.sql",
  "utf8",
);
...
describe("CHRISMED events and contrast contract", () => {
  it("exposes Eventos in the primary navigation and footer", () => {
    expect(shell.match(/to:\s*["']\/chrismed\/eventos["']/g)).toHaveLength(2);
  });

  it("publishes an events-only route with its own registration action", () => {
    expect(route).toMatch(/createFileRoute\(["']\/chrismed\/eventos["']\)/);
  ...
  it("keeps event registrations isolated from clinical scheduling", () => {
    expect(eventFunctions).toMatch(/rpc\(\s*["']chrismed_list_public_events["'] as never/);
    expect(eventFunctions).toMatch(/rpc\(\s*["']chrismed_register_event["'] as never/);
