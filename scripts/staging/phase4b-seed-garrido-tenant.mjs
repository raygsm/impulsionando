#!/usr/bin/env node
/**
 * Seed Garrido tenant row on STAGING for Phase 4B-7 low-risk pilot resolve.
 */
...
const HOST = "garrido.impulsionando.com.br";
...
function fail(msg) {
  console.error(`staging:seed:garrido-tenant FAIL — ${msg}`);
  process.exit(1);
}
...
const { data: updated, error: updErr } = await admin
  .from("companies")
  .update({
    name: target.name?.toLowerCase().includes("garrido")
      ? target.name
      : "Imobiliária Garrido (staging seed)",
    subdomain: SUBDOMAIN,
    domain: HOST,
    is_active: true,
