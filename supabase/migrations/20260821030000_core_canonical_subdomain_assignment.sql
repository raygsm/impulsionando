-- Canonical client subdomain assignment for both self-service and Core admin flows.
-- One operation keeps companies, communication tenant, tenant identity and
-- enrollment metadata aligned, while DNS/SSL reconciliation remains external.
...
begin
  ...
  if exists(
    select 1 from public.core_tenant_identity i
    where i.subdomain=v_slug and i.company_id is distinct from p_company_id
  ) or exists(
  ...
  select i.subdomain into v_old_slug
  from public.core_tenant_identity i
  ...
  set slug=v_slug,
      ...
      settings=coalesce(settings,'{}'::jsonb)||jsonb_build_object(
        'core_auto_enrolled',true,
        'canonical_subdomain',v_slug
      ),
      updated_at=now()
  ...
  insert into public.core_tenant_identity(
    ...
    jsonb_build_object(
      'source','core_canonical_subdomain',
  ...
  set subdomain=v_slug
  where id=p_company_id and subdomain is distinct from v_slug;
...
begin
  ...
  insert into public.companies(name,legal_name,document,email,phone,is_master,is_active,is_demo,status,company_kind)
  values(
    trim(p_name),
  ...
  if v_requested is not null then
    v_slug := public.core_set_company_subdomain(v_company.id,v_requested);
  else
    v_slug := v_enrollment->>'subdomain';
  end if;

  select t.id into v_tenant_id
  from public.communication_tenants t
  where t.company_id=v_company.id and t.active=true and t.deleted_at is null
  ...
  values(
    ...
    jsonb_build_object('actor_user_id',v_user,'source','self_service_onboarding','billing_required',true,'relationship_required',true,'automatic_dns_reconcile',true)
  );
  ...
  return jsonb_build_object(
    ...
    'tenant_id',v_tenant_id,
    'subdomain',v_slug,
    'lifecycle_status',(select lifecycle_status from public.core_client_enrollment where company_id=v_company.id),
