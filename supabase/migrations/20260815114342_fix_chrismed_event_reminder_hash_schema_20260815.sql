...
begin
  ...
  with recipient_source as (
    ...
    from public.chrismed_event_registrations r where r.status = 'confirmed'
    union
    select i.event_id, lower(trim(i.invitee_email)) as email, i.invitee_name as recipient_name
    from public.chrismed_event_invitations i where i.status = 'accepted'
  ), recipients as (
    ...
    select e.id as event_id,e.title as event_name,e.starts_at,e.venue_name,e.venue_address,e.city,
           r.email,r.recipient_name,x.reminder_key,x.threshold
    from public.chrismed_events e
    join recipients r on r.event_id = e.id
    cross join (values ('72h'::text,interval '72 hours'),('24h'::text,interval '24 hours'),('2h'::text,interval '2 hours')) as x(reminder_key,threshold)
    where e.status='published' and e.starts_at>p_now
    ...
    select v_company_id,'event_reminder','email',d.email,
      jsonb_build_object('event_id',d.event_id,'recipient_name',d.recipient_name,'event_name',d.event_name,'starts_at',d.starts_at,'venue_name',d.venue_name,'venue_address',d.venue_address,'city',d.city,'event_url','https://chrismed.impulsionando.com.br/eventos','reminder_key',d.reminder_key),
      'chrismed:event-reminder:'||d.event_id::text||':'||encode(extensions.digest(d.email::text,'sha256'::text),'hex')||':'||d.reminder_key,
