-- Delivery contract completion for CHRISMED.
-- The communication templates in the historical migration predate the
-- universal communication Core in a clean replay. Seed them only when that
-- Core is already available; a later idempotent migration guarantees them.
do $$
declare
  v_tenant uuid;
  v_brand uuid;
  v_template uuid;
begin
  if to_regclass('public.communication_tenants') is not null
     and to_regclass('public.communication_brands') is not null
     and to_regclass('public.communication_templates') is not null
     and to_regclass('public.communication_template_versions') is not null then
    ...
    raise notice 'CHRISMED delivery communication templates deferred until universal communication Core exists';
  end if;
end $$;
