-- CHRISMED recording consent must use the current professional source.
...
begin
  ...
  insert into public.chrismed_teleconsult_recording_consents(
    ...
    p_appointment_id,v_uid,p_party_role,v_decision,p_consent_version,p_consent_text_hash,now(),p_user_agent_hash,p_ip_hash
  )
  ...
  return jsonb_build_object(
    'decision',v_decision,
    'recording_ready',exists(
      select 1 from public.chrismed_teleconsult_recordings r
      where r.appointment_id=p_appointment_id and r.status='ready'
    )
  );
end;
$$;
