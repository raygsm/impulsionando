...
-- The June Agenda migration created this table with a smaller contract. CREATE
-- TABLE IF NOT EXISTS does not add later columns, so clean replays must converge
-- the historical shape before indexes and Pega Agenda functions reference it.
...
-- Legacy Agenda pointed appointment_id to agenda_appointments. Pega Agenda moves
-- CHRISMED appointments, so converge that foreign key deterministically.
alter table public.agenda_open_slots drop constraint if exists agenda_open_slots_appointment_id_fkey;
alter table public.agenda_open_slots add constraint agenda_open_slots_appointment_id_fkey foreign key (appointment_id) references public.chrismed_appointments(id) on delete set null;
...
-- June allowed seen/locked but not cancelled; the current atomic claim logic marks
-- competing offers as cancelled. Replace only the historical default status check.
alter table public.agenda_slot_offers drop constraint if exists agenda_slot_offers_status_check;
alter table public.agenda_slot_offers add constraint agenda_slot_offers_status_check check(status in ('sent','accepted','declined','expired','cancelled'));
