...
-- Permissao de gestao: admin (master) ou financeiro da respectiva empresa; staff/superadmin do Core tambem pode administrar.
-- O preco final nunca e confiado ao frontend: deve ser resolvido pela RPC core_resolve_customer_price.
...
create table if not exists public.core_customer_discount_coupons (
  ...
  suspended_by uuid null references auth.users(id),
...
create index if not exists idx_core_coupon_events_coupon on public.core_coupon_events(coupon_id, created_at desc);
  ...
  select p_user_id is not null and (
    ...
    or exists (
      ...
      where ur.user_id=p_user_id
        ...
        and ur.role::text in ('admin','financeiro')
    )
  )
$$;
...
returns boolean language sql stable security definer set search_path=public,auth as $$
  select exists (
    ...
    where up.user_id=p_customer_user_id and up.company_id=p_company_id and coalesce(up.is_active,true)=true
  ) or exists (
    select 1 from public.communication_contacts cc
    join public.communication_tenants ct on ct.id=cc.tenant_id
    where cc.user_id=p_customer_user_id and ct.company_id=p_company_id and ct.active=true and ct.deleted_at is null
  )
$$;
...
begin
  ...
  return query
  with candidates as (
    select up.user_id, coalesce(up.display_name,up.email,'Cliente')::text as display_name,
           nullif(regexp_replace(coalesce((to_jsonb(up)->>'cpf'),''),'[^0-9]','','g'),'') as cpf,
           up.email::text as email, null::uuid as contact_id
      from public.user_profiles up
     where up.company_id=p_company_id and coalesce(up.is_active,true)=true
    union all
    select cc.user_id, coalesce(cc.display_name,'Cliente')::text,
           nullif(regexp_replace(coalesce(cc.attributes->>'cpf',''),'[^0-9]','','g'),'') as cpf,
           coalesce(cc.attributes->>'email','')::text, cc.id
    ...
    select distinct on (c.user_id) c.* from candidates c
    where c.user_id is not null and (
      c.display_name ilike '%'||q||'%'
      or c.email ilike '%'||q||'%'
...
begin
  ...
  values(p_company_id,v_id,p_customer_user_id,'COUPON_CREATED',jsonb_build_object('service_ref',trim(p_service_ref),'service_name',trim(p_service_name),'validity_type',p_validity_type,'expires_at',v_expires));
...
declare c public.core_customer_discount_coupons%rowtype; nt text; et text;
begin
  ...
  update public.core_customer_discount_coupons set status=p_status,reason=coalesce(nullif(trim(p_reason),''),reason),
  ...
  insert into public.core_coupon_events(company_id,coupon_id,customer_user_id,event_type,actor_user_id,metadata) values(c.company_id,c.id,c.customer_user_id,et,auth.uid(),jsonb_build_object('reason',p_reason));
  insert into public.core_coupon_notification_outbox(company_id,coupon_id,customer_user_id,notification_type,payload) values(c.company_id,c.id,c.customer_user_id,nt,jsonb_build_object('service_name',c.service_name_snapshot,'reason',p_reason));
  return true;
end $$;
...
begin
  for r in select * from public.core_customer_discount_coupons where status='ACTIVE' and expires_at is not null and expires_at<=now() for update skip locked loop
    ...
    insert into public.core_coupon_notification_outbox(company_id,coupon_id,customer_user_id,notification_type,payload) values(r.company_id,r.id,r.customer_user_id,'COUPON_EXPIRED',jsonb_build_object('service_name',r.service_name_snapshot));
    n:=n+1;
  end loop;
  return n;
end $$;
...
begin
  ...
  update public.core_customer_discount_coupons set status='EXPIRED',updated_at=now() where company_id=p_company_id and customer_user_id=p_customer_user_id and service_ref=p_service_ref and status='ACTIVE' and expires_at is not null and expires_at<=now();
  select * into c from public.core_customer_discount_coupons
   where company_id=p_company_id and customer_user_id=p_customer_user_id and service_ref=p_service_ref and status='ACTIVE' and starts_at<=now() and (expires_at is null or expires_at>now())
   order by created_at desc limit 1;
  if c.id is null then return jsonb_build_object('eligible',false,'reason','NO_ACTIVE_COUPON','base_price_cents',p_base_price_cents,'final_price_cents',p_base_price_cents,'discount_cents',0); end if;
  if c.discount_type='PERCENT' then final_cents:=greatest(0,round(p_base_price_cents*(1-(c.discount_percent/100)))::bigint); else final_cents:=greatest(0,c.fixed_price_cents); end if;
  ...
  return jsonb_build_object('eligible',true,'coupon_id',c.id,'discount_type',c.discount_type,'discount_percent',c.discount_percent,'fixed_price_cents',c.fixed_price_cents,'base_price_cents',p_base_price_cents,'final_price_cents',final_cents,'discount_cents',discount_cents,'expires_at',c.expires_at);
end $$;
...
grant select on public.core_customer_discount_coupons to authenticated;
grant select on public.core_coupon_events to authenticated;
grant select on public.core_coupon_notification_outbox to authenticated;
...
comment on table public.core_customer_discount_coupons is 'Cupons nominais nativos do Core, vinculados obrigatoriamente a usuario cadastrado e a servico. Preco final resolvido server-side.';
comment on table public.core_coupon_notification_outbox is 'Outbox universal para avisos automaticos de criacao, suspensao, reativacao, expiracao e revogacao de cupons.';
