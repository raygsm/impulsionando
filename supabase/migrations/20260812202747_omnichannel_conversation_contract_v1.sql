...
begin
  ...
  if v_contact_id is null then
    ...
    values(v_tenant_id,p_display_name,jsonb_build_object('first_channel',p_channel))
    ...
    update public.communication_contacts
      set display_name=coalesce(display_name,p_display_name),updated_at=now()
  ...
  order by c.last_message_at desc nulls last,c.opened_at desc
  limit 1;
  ...
  if v_message_id is null then
    insert into public.communication_conversation_messages(conversation_id,tenant_id,agent_id,contact_id,endpoint_id,channel,provider,direction,author_type,provider_message_id,body_text,content,status,occurred_at)
    values(v_conversation_id,v_tenant_id,v_agent_id,v_contact_id,v_endpoint_id,p_channel,coalesce(nullif(p_provider,''),'unbound'),'INBOUND','CONTACT',p_provider_message_id,p_body_text,coalesce(p_metadata,'{}'::jsonb),'RECEIVED',now())
...
begin
  ...
  insert into public.communication_conversation_messages(conversation_id,tenant_id,agent_id,contact_id,endpoint_id,channel,provider,direction,author_type,provider_message_id,body_text,content,status,occurred_at)
  values(v_c.id,v_c.tenant_id,v_c.agent_id,v_c.contact_id,p_endpoint_id,p_channel,coalesce(nullif(p_provider,''),'unbound'),'OUTBOUND','AGENT',p_provider_message_id,p_body_text,coalesce(p_metadata,'{}'::jsonb),p_status,now())
  ...
  update public.communication_conversation_channels
    ...
    where conversation_id=v_c.id and channel=p_channel and provider=coalesce(nullif(p_provider,''),'unbound') and active=true;
...
create or replace function public.communication_link_verified_identity(
...
begin
  if p_source_contact_id=p_target_contact_id then
    return jsonb_build_object('merged',false,'reason','same_contact');
  end if;
  if not exists(select 1 from public.communication_contact_identities where contact_id=p_source_contact_id and verified=true) then
    raise exception 'source_identity_not_verified';
  end if;
  if not exists(select 1 from public.communication_contact_identities where contact_id=p_target_contact_id and verified=true) then
    raise exception 'target_identity_not_verified';
  end if;
  update public.communication_contact_identities set contact_id=p_target_contact_id,updated_at=now() where contact_id=p_source_contact_id;
  update public.communication_conversations set contact_id=p_target_contact_id,context=context||jsonb_build_object('identity_merge_reason',p_reason),updated_at=now() where contact_id=p_source_contact_id;
  update public.communication_contacts set merged_into_contact_id=p_target_contact_id,updated_at=now() where id=p_source_contact_id;
  return jsonb_build_object('merged',true,'source_contact_id',p_source_contact_id,'target_contact_id',p_target_contact_id);
end;
$$;
revoke all on function public.communication_link_verified_identity(uuid,uuid,text) from public,anon,authenticated;
grant execute on function public.communication_link_verified_identity(uuid,uuid,text) to service_role;
