...
begin
  ...
  v_should_enqueue := (tg_op='INSERT' and new.status='confirmed') or (tg_op='UPDATE' and old.status is distinct from new.status and new.status='confirmed');
  ...
  if v_should_enqueue then
    ...
    on conflict(idempotency_key) do update set recipient=excluded.recipient,payload=excluded.payload,status='pending',available_at=excluded.available_at,attempts=0,last_error=null,sent_at=null,updated_at=v_now;
  elsif tg_op='UPDATE' and old.status is distinct from new.status and new.status in ('waitlisted','cancelled') then
...
update public.chrismed_communication_outbox o
   set status='dead_letter',
       last_error=case when r.status='cancelled' then 'registration_cancelled' when r.status='waitlisted' then 'registration_waitlisted' else 'registration_not_confirmed' end,
       updated_at=now()
