...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: cred, error: credError } = await service.from('mpago_credentials').select('access_token_secret_name,webhook_secret_name,active').eq('company_id',companyId).eq('active',true).limit(1).maybeSingle();
    ...
    const { data: event, error: eventError } = await service.from('mpago_webhook_events').upsert({company_id:companyId,event_type:eventType,mp_event_id:requestId,mp_resource_id:resourceId,action:payload.action??null,raw_payload:payload,signature_valid:true,processed:false},{onConflict:'mp_event_id,event_type'}).select('id,processed').single();
    ...
    if(eventType==='payment'&&resourceId){
      ...
      const {data:payment,error:paymentError}=await service.from('mpago_payments').update(patch).eq('company_id',companyId).eq('mp_payment_id',resourceId).select('*').maybeSingle();
      if(paymentError) throw paymentError;

      if(payment?.context_type==='chrismed_appointment'&&payment.context_id&&companyId===CHRISMED_COMPANY_ID){
        const appointmentStatus=status==='approved'?'confirmed':['rejected','cancelled','refunded','charged_back'].includes(status)?'cancelled':'pending_payment';
        const {data:appointment,error:appointmentError}=await service.from('chrismed_appointments').update({status:appointmentStatus,updated_at:new Date().toISOString()}).eq('id',payment.context_id).eq('company_id',CHRISMED_COMPANY_ID).eq('payment_id',payment.id).select('id,patient_name,patient_email,starts_at,ends_at').maybeSingle();
        ...
        if(appointment&&appointmentStatus==='confirmed'){
          ...
          const jobs=[
            ...
            ['appointment_confirmed_management','sac@chrismed.com.br',new Date().toISOString(),`appointment:${appointment.id}:management:email`],
          ];
          for(const[eventCode,recipient,availableAt,idempotencyKey]of jobs){await service.from('chrismed_communication_outbox').upsert({company_id:CHRISMED_COMPANY_ID,event_code:eventCode,channel:'email',recipient,payload:basePayload,idempotency_key:idempotencyKey,status:'pending',attempts:0,available_at:availableAt,from_email:'sac@chrismed.com.br',reply_to_email:'sac@chrismed.com.br'},{onConflict:'idempotency_key',ignoreDuplicates:true});}
        }
      }
    }
    ...
    return json({ok:true});
  }catch(error){console.error('[mpago-webhook]',error);return json({error:'webhook_processing_failed'},500);}
});
