...
const labels: Record<string, { subject: string; title: string; cta: string }> = {
  signup: {
    subject: "Confirme seu cadastro — CHRISMED",
    title: "Confirme seu endereço de e-mail",
    cta: "Confirmar cadastro",
  },
  recovery: {
    subject: "Redefinição de senha — CHRISMED",
    title: "Redefina sua senha",
    cta: "Redefinir senha",
  },
  magiclink: {
    subject: "Seu acesso seguro — CHRISMED",
    title: "Acesse sua conta CHRISMED",
    cta: "Acessar com segurança",
  },
  invite: {
    subject: "Convite para a CHRISMED",
    title: "Você recebeu um convite",
    cta: "Aceitar convite",
  },
  email_change: {
    subject: "Confirme a alteração de e-mail — CHRISMED",
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Fail closed: this staged hook may send CHRISMED mail only when Auth supplied
    // an allow-listed application origin. It must not trust user metadata.
    if (redirect.protocol !== "https:" || redirect.hostname !== "chrismed.impulsionando.com.br") {
    ...
    const username = requiredSecret("CHRISMED_SMTP_USERNAME");
    const password = requiredSecret("CHRISMED_SMTP_PASSWORD");
    ...
    await transporter.sendMail({
      from: `"CHRISMED" <${username}>`,
      to: recipient,
      replyTo: "sac@chrismed.com.br",
      subject: copy.subject,
      text: `${copy.title}\n\n${copy.cta}: ${actionUrl.toString()}\n\nSe você não solicitou esta ação, ignore esta mensagem.`,
      html: `<!doctype html><html lang="pt-BR"><body style="margin:0;background:#f4f7f7;font-family:Arial,sans-serif;color:#153b3a"><table role="presentation" width="100%" cellspacing="0" cellpadding="0"><tr><td align="center" style="padding:32px 16px"><table role="presentation" width="100%" style="max-width:560px;background:#fff;border-radius:16px;border:1px solid #dbe7e6"><tr><td style="padding:32px"><div style="font-size:24px;font-weight:700;color:#006b68">CHRISMED</div><h1 style="font-size:22px;margin:24px 0 12px">${escapeHtml(copy.title)}</h1><p style="line-height:1.6">Use o botão abaixo para concluir esta ação com segurança.</p><p style="margin:28px 0"><a href="${safeUrl}" style="display:inline-block;background:#006b68;color:#fff;text-decoration:none;padding:14px 22px;border-radius:999px;font-weight:700">${escapeHtml(copy.cta)}</a></p><p style="font-size:13px;line-height:1.5;color:#526665">Se você não solicitou esta ação, ignore esta mensagem.</p><p style="font-size:12px;color:#71807f;margin-top:28px">CHRISMED · sac@chrismed.com.br</p></td></tr></table></td></tr></table></body></html>`,
    });
