...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: session, error: sessionError } = await service
      ...
      .select('id,plan_id,contact_name,contact_company,contact_email,contact_phone,contact_doc,setup_amount,recurring_amount,due_day,cycle,terms_version,terms_hash,accepted_at,accepted_user_id,status,idempotency_key,mpago_payment_id,billing_contract_id,expires_at,metadata')
    ...
    const { data: plan, error: planError } = await service
      .from('billing_plans')
      .select('id,code,name,setup_fee,recurring_amount,cycle,due_day,is_active,show_in_checkout,allow_direct_checkout,status_comercial')
      .eq('id', session.plan_id)
      .eq('is_active', true)
      .maybeSingle();

    if (planError || !plan) return json({ error: 'plan_not_available' }, 409);
    ...
    const { data: cred, error: credError } = await service
      ...
      .select('access_token_secret_name,active,environment')
      .eq('company_id', IMPULSIONANDO_COMPANY_ID)
      .eq('active', true)
      .eq('environment', 'production')
      .limit(1)
      .maybeSingle();
    if (credError || !cred?.access_token_secret_name) return json({ error: 'payment_provider_not_configured' }, 503);
    ...
    const notificationUrl = `${supabaseUrl.replace(/\/$/, '')}/functions/v1/mpago-webhook?company_id=${encodeURIComponent(IMPULSIONANDO_COMPANY_ID)}`;
    ...
    if (body.payment_method === 'pix') {
      mpBody = {
        ...
        payer: {
          ...
          first_name: String(session.contact_name ?? '').trim().split(' ')[0] || undefined,
          last_name: String(session.contact_name ?? '').trim().split(' ').slice(1).join(' ') || undefined,
          identification: session.contact_doc ? { type: 'CNPJ', number: String(session.contact_doc).replace(/\D/g, '') } : undefined,
        },
        metadata: { checkout_session_id: session.id, plan_id: plan.id, plan_code: plan.code },
      };
    } else {
      ...
      mpBody = {
        ...
        token: body.token,
        description,
        installments: Math.max(1, Math.min(Number(body.installments ?? 1), 12)),
    ...
    const paymentRow = {
      ...
      payer_doc: session.contact_doc,
      context_type: 'billing_checkout_session',
      context_id: session.id,
      metadata: { plan_id: plan.id, plan_code: plan.code, setup_cents: setupCents, recurring_cents: recurringCents },
    ...
    const { error: sessionUpdateError } = await service
      .from('billing_checkout_sessions')
      .update({ mpago_payment_id: payment.id, status: payment.status === 'approved' ? 'paid' : 'pending_payment', updated_at: new Date().toISOString() })
      .eq('id', session.id)
