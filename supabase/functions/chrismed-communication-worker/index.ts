...
const TEMPLATE_MAP: Record<string,string> = {
  ...
  "chrismed.appointment.checkin.welcome":"appointment.checkin.welcome",
  "chrismed.appointment.survey":"appointment.survey",
  ...
  "chrismed.event.survey":"event.survey",
  ...
  "professional.registration.approved":"professional.registration.approved",
  "professional.registration.rejected":"professional.registration.rejected",
  "professional.invitation":"professional.invitation",
  "professional.suspended":"professional.suspended",
  "professional.reactivated":"professional.reactivated",
  "company.invitation":"company.invitation",
  "patient.invitation":"patient.invitation",
  "account.suspended":"account.suspended",
  "account.reactivated":"account.reactivated",
  "payment.failed":"payment.failed",
  "occupational.document.expiring":"occupational.document.expiring",
...
Deno.serve(async(req:Request)=>{
  ...
  for(const row of rows){try{
    ...
    const copies=Array.from(new Set(managementCopies)).filter(e=>e&&e!==row.recipient.toLowerCase());if(copies.length){const sensitive=SENSITIVE_PREFIXES.some(p=>code.startsWith(p));if(sensitive&&clinicalSensitiveMode==="metadata_only"){await transporter.sendMail({from:`"CHRISMED" <${smtpUser}>`,to:managementPrimary,cc:copies,replyTo:"sac@chrismed.com.br",subject:`[CHRISMED · Cópia gerencial] ${code}`,text:`Uma comunicação CHRISMED foi processada.\n\nEvento: ${code}\nReferência operacional: ${row.id}\n\nPor política de minimização de dados, esta cópia gerencial não replica conteúdo assistencial ou conversacional sensível.`});}else await transporter.sendMail({from:`"CHRISMED" <${smtpUser}>`,to:managementPrimary,cc:copies,replyTo:"sac@chrismed.com.br",subject:`[CHRISMED · Cópia gerencial] ${subject}`,text,html});}
    const {error:se}=await db.from("chrismed_communication_outbox").update({status:"sent",sent_at:new Date().toISOString(),last_error:null,updated_at:new Date().toISOString()}).eq("id",row.id).eq("status","processing");if(se)throw new Error("outbox_finalize_sent_failed");
    if(code==="conversation_export_authenticated"&&row.payload?.export_request_id){const {error:xe}=await db.rpc("chrismed_mark_conversation_export_sent",{p_request_id:String(row.payload.export_request_id)});if(xe)throw new Error(`conversation_export_finalize_failed:${xe.message}`);}
    sent++;
