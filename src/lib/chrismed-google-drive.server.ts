...
function sign(payloadB64: string) {
  return createHmac("sha256", requiredEnv("GOOGLE_DRIVE_CLIENT_SECRET"))
    .update(payloadB64)
    .digest("base64url");
}
...
function verifyState(state: string): OAuthState {
  ...
  if (payload.companyId !== CHRISMED_COMPANY_ID || !payload.userId || payload.exp < Date.now()) {
    throw new Error("expired_or_invalid_oauth_state");
  }
  return payload;
}
...
export async function completeGoogleDriveOAuth(input: { code: string; state: string }) {
  ...
  if (!tokenResponse.ok || !tokenJson.access_token) {
    throw new Error(tokenJson.error_description || tokenJson.error || "google_token_exchange_failed");
  }
  ...
  const { data: connectionId, error } = await (supabaseAdmin as any).rpc(
    "client_drive_store_google_refresh_token",
    {
  ...
  return {
    ...
    accountEmail,
    displayName: aboutJson.user?.displayName ?? null,
