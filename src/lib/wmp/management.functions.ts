...
async function tenantId(supabase: any) {
...
export const getWmpOperations = createServerFn({ method:'POST' })
  ...
  .handler(async ({ context }) => {
    const id = await tenantId(context.supabase)
    ...
    for (const [key, result] of Object.entries(all)) if ((result as any).error) throw new Error(`${key}: ${(result as any).error.message}`)
...
export const updateWmpOperationalStatus = createServerFn({ method:'POST' })
  .middleware([requireSupabaseAuth])
  .inputValidator((d:{table:string;id:string;status:string}) => z.object({table:tableSchema,id:z.string().uuid(),status:z.string().trim().min(1).max(40)}).parse(d))
  .handler(async ({ data, context }) => {
    const id = await tenantId(context.supabase)
    const { error } = await context.supabase.from(data.table).update({status:data.status,updated_at:new Date().toISOString()}).eq('tenant_id',id).eq('id',data.id)
    if (error) throw error
    return {ok:true}
  })
...
export const updateWmpEquipment = createServerFn({ method:'POST' })
  .middleware([requireSupabaseAuth])
  .inputValidator((d:{id:string;commercial_value_cents?:number;quantity_available?:number;status?:string}) => z.object({id:z.string().uuid(),commercial_value_cents:z.number().int().min(0).optional(),quantity_available:z.number().int().min(0).optional(),status:z.string().trim().min(1).max(40).optional()}).parse(d))
  .handler(async ({ data, context }) => {
    const id = await tenantId(context.supabase)
    ...
    const { error } = await context.supabase.from('wmp_equipment_catalog').update(patch).eq('tenant_id',id).eq('id',data.id)
    if (error) throw error
    return {ok:true}
  })
