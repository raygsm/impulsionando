...
export const listWmpEquipmentReferenceData=createServerFn({method:'POST'})
  ...
  .handler(async({data,context})=>{
    ...
    const t=await tenantId(s)
    const {data:set,error:setError}=await s.from('reference_option_sets').select('id').eq('key','wmp_equipment_categories').single()
    if(setError)throw setError
    const [{data:categories,error:categoriesError},{data:manufacturers,error:manufacturersError}]=await Promise.all([
      s.from('reference_options').select('code,label,sort_order').eq('set_id',set.id).eq('active',true).order('sort_order').order('label'),
      s.from('wmp_equipment_manufacturers').select('id,name,website').eq('tenant_id',t).eq('active',true).order('name'),
    ])
    if(categoriesError)throw categoriesError
    if(manufacturersError)throw manufacturersError
    let modelsQuery=s.from('wmp_equipment_models').select('id,manufacturer_id,category,product_line,model,submodel,specifications,reference_url').eq('tenant_id',t).eq('active',true).order('model').limit(1000)
...
export const requestWmpEquipmentReference=createServerFn({method:'POST'})
  ...
  .handler(async({data,context})=>{
    const s:any=context.supabase
    const t=await tenantId(s)
    if(data.category)await equipmentCategory(s,data.category)
    const {data:row,error}=await s.from('wmp_equipment_reference_requests').insert({tenant_id:t,...data,requested_by:context.userId,status:'PENDING'}).select('id,status,created_at').single()
    if(error)throw error
    return row
  })
...
export const searchWmpEquipment=createServerFn({method:'POST'})
  ...
  .handler(async({data,context})=>{
    const s:any=context.supabase
    const t=await tenantId(s)
    let q=s.from('wmp_equipment_catalog').select('id,code,category,name,manufacturer_id,model_id,manufacturer,model,quantity_available,internal_cost_cents,commercial_value_cents,status,owner_type,owner_name,beneficiary_kind').eq('tenant_id',t).order('category').order('name').limit(data.limit??200)
...
export const saveWmpEquipment=createServerFn({method:'POST'})
  ...
  .handler(async({data,context})=>{
    const s:any=context.supabase
    const t=await tenantId(s)
    const category=await equipmentCategory(s,data.category)

    let manufacturer:any=null
    if(data.manufacturer_id){
      const {data:row,error}=await s.from('wmp_equipment_manufacturers').select('id,name').eq('tenant_id',t).eq('id',data.manufacturer_id).eq('active',true).maybeSingle()
      if(error)throw error
    ...
    let model:any=null
    if(data.model_id){
      const {data:row,error}=await s.from('wmp_equipment_models').select('id,manufacturer_id,category,product_line,model,submodel').eq('tenant_id',t).eq('id',data.model_id).eq('active',true).maybeSingle()
      if(error)throw error
      ...
      if(!manufacturer&&row.manufacturer_id){
        const {data:m,error:mError}=await s.from('wmp_equipment_manufacturers').select('id,name').eq('tenant_id',t).eq('id',row.manufacturer_id).eq('active',true).single()
    ...
    if(data.id){
      const{data:row,error}=await s.from('wmp_equipment_catalog').update(payload).eq('tenant_id',t).eq('id',data.id).select().single()
      if(error)throw error
      return row
    }
    const{data:row,error}=await s.from('wmp_equipment_catalog').insert(payload).select().single()
    if(error)throw error
    return row
  })

export const listWmpEquipmentRentals=createServerFn({method:'POST'}).middleware([requireSupabaseAuth]).handler(async({context})=>{const s:any=context.supabase;const t=await tenantId(s);const{data,error}=await s.from('wmp_equipment_rentals').select('id,equipment_id,proposal_id,dj_booking_id,quantity,unit_rental_cents,status,owner_type,owner_name,created_at,wmp_equipment_catalog(name,code,category)').eq('tenant_id',t).order('created_at',{ascending:false}).limit(200);if(error)throw error;return data??[]})
export const listWmpRentalPayouts=createServerFn({method:'POST'}).middleware([requireSupabaseAuth]).handler(async({context})=>{const s:any=context.supabase;const t=await tenantId(s);const{data,error}=await s.from('wmp_equipment_rental_payouts').select('id,rental_id,beneficiary_type,beneficiary_name,amount_cents,status,paid_at,created_at').eq('tenant_id',t).order('created_at',{ascending:false}).limit(200);if(error)throw error;return data??[]})
