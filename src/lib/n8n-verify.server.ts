...
export async function handleN8nVerifyPost(request: Request): Promise<Response> {
  ...
  return Response.json({ ok: true, authorized: true, workflow_name: parsed.workflow_name, event_code: parsed.event_code, tenant_slug: parsed.tenant_slug, correlation_id: parsed.correlation_id, payload: parsed })
}

export function handleN8nVerifyGet(): Response {
  return Response.json({ ok: true, auth: 'hmac', purpose: 'n8n_dispatch_verifier' })
}
