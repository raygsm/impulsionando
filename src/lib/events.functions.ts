...
const EventInput = z.object({
  ...
  transferDeadlineHours: z.number().int().min(0).max(8760).nullable().optional(),
  transferFeeCents: z.number().int().min(0).max(1_000_000).default(0),
  transferRequiresDocument: z.boolean().default(false),
...
export const upsertEvent = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const row = {
      ...
      transfer_deadline_hours: data.transferDeadlineHours ?? null,
