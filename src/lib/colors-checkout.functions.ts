...
const optionalCpf = z.string().trim().max(20).optional().superRefine((value, ctx) => {
  if (value && !isValidCPF(value)) ctx.addIssue({ code: z.ZodIssueCode.custom, message: "CPF inválido." });
});
...
function hashCpf(v?: string) {
  ...
  if (!isValidCPF(clean)) return null;
...
export const startColorsCheckout = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (emailN) {
      const { data: existing } = await sb.from("colors_contacts").select("id").eq("email_normalized", emailN).maybeSingle();
      contactId = existing?.id ?? null;
    }
    if (!contactId && phoneN) {
      const { data: existing } = await sb.from("colors_contacts").select("id").eq("whatsapp_normalized", phoneN).maybeSingle();
      contactId = existing?.id ?? null;
    }
    ...
    if (contactId) {
      ...
      const { data: inserted, error } = await sb.from("colors_contacts").insert(contactPayload).select("id").single();
    ...
    if (data.affiliateCode) {
      const { data: aff } = await sb.from("colors_affiliates").select("id").eq("code", data.affiliateCode).maybeSingle();
    ...
    return { opportunityId: opp.id, colorsCheckoutId: opp.colors_checkout_id, contactId, affiliateId };
  });
