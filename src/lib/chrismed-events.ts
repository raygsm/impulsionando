...
const RegistrationInput = z.object({
  ...
  referredEmail: z.union([z.string().trim().email().max(320), z.literal("")]).optional(),
  website: z.string().max(0).optional(),
});

export type PublicChrismedEvent = {
  id: string; slug: string; title: string; summary: string | null; description: string | null;
  coverUrl: string | null; venueName: string | null; venueAddress: string | null; city: string | null;
  startsAt: string; endsAt: string; capacity: number; seatsRemaining: number; registrationOpen: boolean;
  isPast: boolean; contractorType: string | null; contractorName: string | null; eventKind: string | null;
};
...
export async function listPublicChrismedEvents() {
  ...
  return ((events ?? []) as unknown as Array<Record<string, unknown>>).map((event) => {
    ...
    const isPast = new Date(String(event.ends_at)).getTime() < current;
    return {
      ...
      startsAt: String(event.starts_at), endsAt: String(event.ends_at), capacity: Number(event.capacity), seatsRemaining,
...
export async function registerForChrismedEvent(input: unknown) {
  ...
  if (data.referredEmail && data.referredEmail.toLowerCase() === data.email.toLowerCase()) throw new Error("A indicação precisa usar o e-mail de outro profissional.");
  const { data: result, error } = await supabase.rpc("chrismed_register_event" as never, {
    ...
    p_referred_email: data.referredEmail || null,
  } as never);
  if (error) {
    const known = ["Já existe uma solicitação ativa", "Evento indisponível", "Evento lotado", "Inscrições ainda não abertas", "Inscrições encerradas", "A participação é individual", "E-mail indicado inválido"];
    throw new Error(known.find((item) => error.message.includes(item)) ?? "Não foi possível enviar sua solicitação. Tente novamente.");
  }
...
export async function getChrismedEventCredential(token: string) {
  ...
  return {
    registrationId: String(row.registration_id), registrationCode: String(row.registration_code), attendeeName: String(row.attendee_name),
    attendeeEmail: String(row.attendee_email), eventId: String(row.event_id), eventTitle: String(row.event_title), startsAt: String(row.starts_at),
    endsAt: String(row.ends_at), venueName: row.venue_name ? String(row.venue_name) : null,
    venueAddress: row.venue_address ? String(row.venue_address) : null, city: row.city ? String(row.city) : null, qrToken: String(row.qr_token),
  };
}