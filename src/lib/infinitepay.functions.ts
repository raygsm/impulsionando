...
function getEnv() {
  ...
  if (!handle || !redirect_url || !webhook_url) {
    throw new Error("InfinitePay não configurado: defina INFINITEPAY_HANDLE, INFINITEPAY_REDIRECT_URL e INFINITEPAY_WEBHOOK_URL.");
  }
...
export const createInfinitePayCheckout = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (data.environment === "demo") {
      ...
      const { data: row, error } = await supabaseAdmin
        ...
        .insert({
          ...
          amount: data.items.reduce((s, i) => s + i.price * i.quantity, 0),
          status: "paid",
          paid_amount: data.items.reduce((s, i) => s + i.price * i.quantity, 0),
          capture_method: "demo",
    ...
    return { ok: true as const, demo: false, order_nsu, checkout_url: json.url as string };
  });
...
export const getInfinitePayPayment = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((data: unknown) => z.object({ order_nsu: z.string().min(1) }).parse(data))
  .handler(async ({ data, context }) => {
    const { userId } = context;
    const { data: row, error } = await supabaseAdmin
      .from("infinitepay_payments")
    ...
    if (row.user_id && row.user_id !== userId) return { ok: false as const, payment: null };
