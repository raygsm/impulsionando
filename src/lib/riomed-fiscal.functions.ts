...
export const createMpPreferenceForAr = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: cred } = await sb
      ...
      .select("access_token, public_key, environment")
      .eq("company_id", cid)
      .maybeSingle();
    ...
    const body = {
      ...
      notification_url: `${process.env.PUBLIC_APP_URL ?? "https://impulsionando.com.br"}/api/mercadopago/webhook`,
    };
...
export const reconcileArByExternalRef = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: pay } = await sb
      .from("mpago_payments")
      .select("id, status, mp_payment_id, transaction_amount")
      .eq("company_id", cid)
    ...
    if (pay.status === "approved") {
      await sb
        ...
        .update({
          status: "paid",
          paid_amount: Number(pay.transaction_amount ?? ar.amount),
