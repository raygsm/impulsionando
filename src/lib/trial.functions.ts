...
export const requestTrial = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    try {
      ...
      // Flush message_outbox rows generated by trial_create() so that BOTH
      // email and WhatsApp messages go out immediately — never queued and
      // forgotten. Failures here are logged but never block the response.
      try {
        const idStr = String(trialId ?? "");
        if (idStr) {
          const { flushOutboxByReference } = await import("@/lib/outboxFlush.server");
          const flushed = await flushOutboxByReference("trial", idStr);
