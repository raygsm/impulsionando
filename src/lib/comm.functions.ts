...
// -----------------------------------------------------------------------------
// worker (chamado pela rota pública /api/public/comm/tick)
// -----------------------------------------------------------------------------
export async function runCommTick(batchSize = 25): Promise<{
  ...
  const { dispatchByChannel } = await import("./comm/adapters.server");
  ...
  for (const d of batch) {
    ...
    if (result.status === "sent") {
      ...
      const backoff = [30, 120, 600, 3600, 21600][attempts] ?? 21600; // seg
