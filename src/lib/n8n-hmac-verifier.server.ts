...
export async function handleN8nHmacVerifier(request: Request): Promise<Response> {
  ...
  if (!secret) {
    return Response.json({ ok: false, authorized: false, error: 'hmac_not_configured' }, { status: 503 })
  }
