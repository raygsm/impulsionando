...
export const getFinanceOverview = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { error: refreshError } = await sb.rpc("riomed_refresh_overdue", { p_company_id: cid });
    ...
    for (const r of [ar, ap, comm, rules]) if (r.error) throw new Error(r.error.message);
    const sum = (rows: any[], col = "amount") => rows.reduce((s, r) => s + Number(r[col] ?? 0), 0);
    const arRows = ar.data ?? [];
    const apRows = ap.data ?? [];
    const cRows = comm.data ?? [];
    const open = (rows: any[]) => rows.filter((r) => ["open", "partial", "overdue"].includes(r.status));
    ...
    return {
      ...
      kpis: {
        ...
        arOverdue: sum(overdue(arRows), "amount") - sum(overdue(arRows), "paid_amount"),
        apOverdue: sum(overdue(apRows), "amount") - sum(overdue(apRows), "paid_amount"),
...
export const createApInvoice = createServerFn({ method: "POST" })
  ...
  .inputValidator((d: unknown) => z.object({
    supplierId: z.string().uuid().optional(), description: z.string().min(2), amount: z.number().positive(),
    dueDate: z.string(), category: z.string().optional(), notes: z.string().optional(),
  }).parse(d))
  .handler(async ({ data, context }) => {
    const sb = (context as any).supabase;
    const cid = await companyId(context as any);
    const { error } = await sb.from("riomed_ap_invoices").insert({
      company_id: cid, supplier_id: data.supplierId ?? null, description: data.description,
      amount: data.amount, due_date: data.dueDate, category: data.category ?? null, notes: data.notes ?? null,
    });
    if (error) throw new Error(error.message);
    return { ok: true };
  });
...
export const createArInvoiceFromOrder = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: unknown) => z.object({ orderId: z.string().uuid(), dueDays: z.number().int().min(0).max(3650).default(30) }).parse(d))
  .handler(async ({ data, context }) => {
    const sb = (context as any).supabase;
...
export const markInvoicePaid = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: unknown) => z.object({ table: z.enum(["riomed_ar_invoices", "riomed_ap_invoices"]), id: z.string().uuid(), paymentMethod: z.string().optional() }).parse(d))
  .handler(async ({ data, context }) => {
    const sb = (context as any).supabase;
    const cid = await companyId(context as any);
    const { data: row, error: readError } = await sb.from(data.table).select("amount").eq("id", data.id).eq("company_id", cid).maybeSingle();
    if (readError) throw new Error(readError.message);
    if (!row) throw new Error("Documento financeiro não encontrado");
    const { error } = await sb.from(data.table).update({
      status: "paid", paid_amount: row.amount ?? 0, paid_at: new Date().toISOString(), payment_method: data.paymentMethod ?? "manual",
...
export const upsertCommissionRule = createServerFn({ method: "POST" })
  ...
  .inputValidator((d: unknown) => z.object({
    ...
    scope: z.enum(["default", "user", "category"]).default("default"), category: z.string().optional(), ratePct: z.number().min(0).max(100),
  }).parse(d))
  .handler(async ({ data, context }) => {
    const sb = (context as any).supabase;
    const cid = await companyId(context as any);
    const payload: any = { company_id: cid, user_id: data.userId ?? null, scope: data.scope, category: data.category ?? null, rate_pct: data.ratePct, active: true };
    const result = data.id
      ? await sb.from("riomed_commission_rules").update(payload).eq("id", data.id).eq("company_id", cid)
...
export const updateCommissionStatus = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: unknown) => z.object({ id: z.string().uuid(), status: z.enum(["accrued", "approved", "paid", "cancelled"]) }).parse(d))
  .handler(async ({ data, context }) => {
    const sb = (context as any).supabase;
    const cid = await companyId(context as any);
    const patch: any = { status: data.status };
    if (data.status === "paid") patch.paid_at = new Date().toISOString();
    const { data: row, error } = await sb.from("riomed_commissions").update(patch).eq("id", data.id).eq("company_id", cid).select("id").maybeSingle();
    if (error) throw new Error(error.message);
