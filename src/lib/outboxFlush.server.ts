// Server-only helper that flushes pending rows of public.message_outbox
// for a given reference (e.g. a trial id) to the actual delivery channels:
//
//  • email    → push into the existing `transactional_emails` pgmq queue
//               (the /lovable/email/queue/process worker delivers it via
//               sendLovableEmail). This is the same path used by the rest
//               of the project.
//
//  • whatsapp → call Z-API directly via sendWhatsAppText().
//
// After a successful dispatch the message_outbox row is marked `sent`.
// On failure the row stays `queued`, `attempts` is incremented and
// `last_error` is filled so a future retry can pick it up.
//
// IMPORTANT: this helper is intentionally generic and not specific to
// trials. Anything that calls enqueue_message() in Postgres can call
// flushOutboxByReference() right after, to guarantee that BOTH channels
// (email + whatsapp) actually go out — never queued and forgotten.
...
function textToHtml(subject: string, body: string): string {
  // Auto-link URLs so e-mail clients render the magic-link as clickable.
  ...
  <div style="max-width:600px;margin:0 auto;background:#ffffff;border:1px solid #e2e8f0;border-radius:12px;overflow:hidden;">
    ...
    <div style="padding:24px;">
      ...
      <p style="color:#64748b;font-size:12px;margin-top:32px;border-top:1px solid #e2e8f0;padding-top:14px;">
        Você está recebendo esta mensagem porque iniciou um Trial em https://impulsionando.com.br.
      </p>
    </div>
  </div>
</body></html>`;
...
/**
 * Flush all `queued` rows of public.message_outbox that match the given
 * (reference_type, reference_id). Used right after enqueue_message() so
 * the user always gets both channels.
 *
 * Errors are swallowed and logged — we never want a delivery failure to
 * block the user-facing flow (e.g. trial activation).
 */
export async function flushOutboxByReference(
  ...
  const { data: rows, error } = await supabaseAdmin
    .from("message_outbox")
    .select(
      "id, channel, subject, body, recipient_email, recipient_phone, recipient_name, event_code, reference_type, reference_id, attempts",
    )
    ...
    .limit(20);
  if (error) {
    console.error("[flushOutboxByReference] read error:", error.message);
    return { attempted: 0, sent: 0, failed: 0 };
  }
  if (!rows?.length) return { attempted: 0, sent: 0, failed: 0 };
  ...
  for (const row of rows as OutboxRow[]) {
    ...
    const result =
      row.channel === "email" ? await dispatchEmail(row) : await dispatchWhatsApp(row);
    ...
    if (result.ok) {
      ...
      failed++;
      console.error("[flushOutboxByReference] dispatch failed", {
        outbox_id: row.id,
        channel: row.channel,
        event_code: row.event_code,
        error: result.error,
