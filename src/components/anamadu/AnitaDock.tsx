...
import { Bot, Gem, Gift, Hammer, ImagePlus, Search, Send, Sparkles, X } from 'lucide-react';
  ...
  async function sendText(raw: string, suppliedImages?: string[]) {
    ...
    try {
      ...
      setMessages((m) => m.map((message, index) => index === m.length - 1 ? {
        ...message,
        text: 'Não consegui concluir esta etapa agora. Seu briefing continua preservado. A análise orçamentária humana só deve avançar quando as referências e condições forem validadas.',
      } : message));
    } finally { setBusy(false); }
  }

  if (!open) {
    return <button type="button" onClick={() => setOpen(true)} aria-label="Falar com Annita" className="fixed bottom-5 right-5 z-50 flex items-center gap-2 rounded-full bg-stone-950 px-5 py-3 text-sm font-semibold text-white shadow-2xl transition hover:-translate-y-0.5"><Sparkles className="size-4" /> Falar com Annita</button>;
  }

  return <section role="dialog" aria-label="Annita — consultora virtual Ana Madú" className="fixed bottom-4 right-4 z-50 flex h-[min(740px,calc(100dvh-32px))] w-[min(460px,calc(100vw-32px))] flex-col overflow-hidden rounded-[28px] border border-stone-200 bg-[#fffdf8] shadow-2xl">
    <header className="flex items-center gap-3 border-b border-stone-200 bg-stone-950 p-4 text-white"><div className="flex size-11 items-center justify-center rounded-full bg-amber-200 text-stone-950"><Bot className="size-5" /></div><div className="min-w-0 flex-1"><strong className="block text-base">Annita</strong><div className="truncate text-xs text-stone-300">Consultora virtual Ana Madú · visão + catálogo + Ourives</div></div><button type="button" onClick={() => setOpen(false)} aria-label="Fechar Annita" className="rounded-full p-2 hover:bg-white/10"><X className="size-5" /></button></header>
    <div className="flex-1 space-y-3 overflow-y-auto p-4">
      {!messages.length && <><div className="rounded-2xl bg-stone-100 p-4 text-sm leading-relaxed text-stone-700"><div className="mb-1 font-semibold text-stone-950">Sua curadoria começa aqui.</div>Envie texto ou imagens. Eu posso cruzar estilo, ocasião, tipo de peça, referências visuais, pedra e faixa de preço — ou receber seu projeto Ourives aprovado.</div><div className="grid grid-cols-2 gap-2">{ACTIONS.map(({ label, prompt, icon: Icon }) => <button key={label} type="button" onClick={() => void sendText(prompt)} className="min-h-24 rounded-2xl border border-stone-200 bg-white p-3 text-left text-xs font-medium text-stone-800 transition hover:border-stone-400 hover:shadow-sm"><Icon className="mb-2 size-4" />{label}</button>)}</div></>}
    ...
    <form className="border-t border-stone-200 p-3" onSubmit={(event) => { event.preventDefault(); void sendText(input); }}>
      ...
      <div className="flex gap-2"><label className="flex size-12 cursor-pointer items-center justify-center rounded-full border border-stone-300 bg-white" title="Enviar imagens"><ImagePlus className="size-4" /><input type="file" accept="image/*" multiple className="hidden" onChange={(e) => void readImages(e.target.files).then((newImages) => setImages((current) => [...current, ...newImages].slice(0, 3)))} /></label><input value={input} onChange={(e) => setInput(e.target.value)} placeholder="Mensagem ou referência visual..." aria-label="Mensagem para Annita" className="min-w-0 flex-1 rounded-full border border-stone-300 bg-white px-4 py-3 text-base outline-none focus:border-stone-700" /><button disabled={busy || (!input.trim() && !images.length)} aria-label="Enviar" className="flex size-12 items-center justify-center rounded-full bg-stone-950 text-white disabled:opacity-40"><Send className="size-4" /></button></div><p className="mt-2 px-1 text-[10px] leading-relaxed text-stone-500">Até 3 imagens, 2 MB cada. A Annita usa referências para compreender estilo e intenção, sem afirmar autenticidade, procedência, material ou viabilidade sem validação humana.</p>
    </form>
  </section>;
}
