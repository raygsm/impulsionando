...
type FormState = {
  ...
  evento_publico_faixa: string; evento_cep: string; evento_bairro: string; evento_cidade: string; evento_estado: string; evento_endereco: string;
...
const INIT: FormState = {
  ...
  evento_publico_faixa: "", evento_cep: "", evento_bairro: "", evento_cidade: "", evento_estado: "", evento_endereco: "",
...
export function WmpOrcamentoForm() {
  ...
  useEffect(() => {
    ...
    fetch(`https://servicodados.ibge.gov.br/api/v1/localidades/estados/${form.evento_estado}/municipios?orderBy=nome`)
      .then((r) => r.ok ? r.json() : Promise.reject(new Error("Falha ao carregar municípios")))
      .then((rows: Array<{ nome: string }>) => { if (active) setCities(rows.map((x) => x.nome)); })
      .catch(() => { if (active) setCities(form.evento_cidade ? [form.evento_cidade] : []); })
  ...
  async function lookupCep(raw: string) {
    ...
    try {
      ...
      setForm((f) => ({ ...f, evento_cep: cep, evento_endereco: data.logradouro ?? "", evento_bairro: data.bairro ?? "", evento_estado: data.uf ?? "", evento_cidade: data.localidade ?? "" }));
  ...
  function canAdvance() {
    ...
    if (step === 1) return !!form.evento_tipo && !!form.evento_data && !!form.evento_publico_faixa && form.evento_cep.replace(/\D/g, "").length === 8 && !!form.evento_estado && !!form.evento_cidade;
    return true;
  }
  ...
  async function handleSubmit() {
    ...
    try {
      const res = await submitWmpBriefing({ data: {
        ...
        evento_cep: form.evento_cep, evento_bairro: form.evento_bairro, evento_cidade: form.evento_cidade, evento_estado: form.evento_estado, evento_endereco: form.evento_endereco,
  ...
  return <WmpShell breadcrumbs={[{ label: "Orçamento" }]}>
    ...
    <section className="mx-auto max-w-3xl px-4 md:px-6 pb-24 -mt-6"><Stepper step={step}/><div className="wmp-surface p-5 md:p-10 mt-6">
      ...
      {step === 1 && <Grid>
        ...
        <Field label="CEP *"><div className="relative"><input inputMode="numeric" value={form.evento_cep} onChange={(e)=>lookupCep(e.target.value)} placeholder="00000000"/>{cepLoading && <Loader2 className="absolute right-3 top-3 size-4 animate-spin"/>}</div></Field>
        <Field label="Endereço"><input value={form.evento_endereco} onChange={(e)=>update("evento_endereco",e.target.value)}/></Field><Field label="Bairro"><input value={form.evento_bairro} onChange={(e)=>update("evento_bairro",e.target.value)}/></Field>
        <Field label="Estado *"><select value={form.evento_estado} onChange={(e)=>{update("evento_estado",e.target.value);update("evento_cidade","");}}><option value="">Selecione</option>{WMP_UFS.map(([uf,nome])=><option key={uf} value={uf}>{uf} — {nome}</option>)}</select></Field>
        <Field label="Cidade *"><select value={form.evento_cidade} disabled={!form.evento_estado || citiesLoading} onChange={(e)=>update("evento_cidade",e.target.value)}><option value="">{citiesLoading?"Carregando...":"Selecione"}</option>{form.evento_cidade && !cities.includes(form.evento_cidade) && <option value={form.evento_cidade}>{form.evento_cidade}</option>}{cities.map((city)=><option key={city} value={city}>{city}</option>)}</select></Field>
      </Grid>}
