...
import { DemoPlanSwitcher } from "./DemoPlanSwitcher";
...
export function DemoShell({ template }: Props) {
    ...
    <div className="min-h-screen bg-muted/30">
      <header className="sticky top-0 z-20 border-b bg-background/95 backdrop-blur">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center gap-3 px-4 py-3">
          ...
          <DemoPlanSwitcher active={plan} recommended={template.recommendedPlan} onChange={setPlan} />
      ...
      <div className="mx-auto grid max-w-7xl gap-6 px-4 py-6 lg:grid-cols-[240px_1fr]">
        ...
        <main>
          <DemoDashboard template={template} plan={plan} onUnlock={setPlan} />
        </main>
      </div>
    </div>
  );
