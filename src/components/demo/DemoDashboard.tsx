...
const SEVERITY: Record<string, { icon: any; className: string }> = {
...
export function DemoDashboard({ template, plan, onUnlock }: Props) {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-wrap items-center gap-2">
        {actions.map((a) => (
          <Button key={a.id} variant={a.id === actions[0].id ? "default" : "outline"} size="sm">
            {a.label}
          </Button>
        ))}
      </div>
