      ...
      GET: async () => {
        ...
        if (!resp.ok) {
          ...
          if (resp.status === 503 && lower.includes("core sync") && lower.includes("configurado")) {
            ...
            message =
              'CHRISMED respondeu "Core sync não configurado": o secret IMPULSIONANDO_CORE_SECRET não está presente no projeto CHRISMED.';
            hint =
              "Adicione o secret IMPULSIONANDO_CORE_SECRET no projeto CHRISMED com EXATAMENTE o mesmo valor cadastrado no Core e publique o CHRISMED.";
          } else if (
            resp.status === 401 ||
            ...
            reason = "hmac_invalid";
            message =
              "CHRISMED rejeitou a assinatura HMAC (401): o valor do secret IMPULSIONANDO_CORE_SECRET no CHRISMED é diferente do cadastrado no Core.";
            hint =
              "Confirme que o secret IMPULSIONANDO_CORE_SECRET tem EXATAMENTE o mesmo valor nos dois projetos (Core e CHRISMED).";
          }
          ...
          return Response.json(
            { ok: false, reason, status: resp.status, hint, body: body.slice(0, 500) },
            { status: 502 },
          );
        }
        ...
        if (eventos.length === 0) {
          return Response.json({
            ...
            count: 0,
            since,
            nextSince: payload.next_since ?? since,
          });
        }
