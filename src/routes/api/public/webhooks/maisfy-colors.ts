...
const str=(...values:unknown[])=>{for(const v of values){if(typeof v==="string"&&v.trim())return v.trim();}return undefined;};
...
export const Route=createFileRoute("/api/public/webhooks/maisfy-colors")({server:{handlers:{
  ...
  POST:async({request})=>{
    ...
    const customer=d.customer??{};const parameters=d.parameters??{};const product=d.product??{};const payment=d.payment??d.transaction??{};
    const paymentUrl=str(d.payment_url,d.checkout_url,d.url,payment.payment_url,payment.checkout_url,payment.url);
    const pixCopyPaste=str(d.pix_copy_paste,d.pix_code,d.qr_code,payment.pix_copy_paste,payment.pix_code,payment.qr_code);
    const pixQrCode=str(d.pix_qr_code,d.qr_code_base64,payment.pix_qr_code,payment.qr_code_base64);
    const paymentMethod=str(d.payment_type,d.payment_method,payment.type,payment.method);
