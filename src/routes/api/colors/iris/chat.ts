...
function safeCheckoutId(value:unknown){const id=typeof value==="string"?value.trim():"";return /^col_[A-Za-z0-9_-]{6,100}$/.test(id)?id:null;}
...
async function buildIrisSystemPrompt(colorsCheckoutId:string|null) {
  ...
  let checkoutContext="Nenhum checkout desta sessão foi identificado.";
  if(colorsCheckoutId){const {data:opp}=await sb.from("colors_opportunities").select("colors_checkout_id,product_name,stage,external_status,offer_url,next_best_action,metadata,updated_at").eq("colors_checkout_id",colorsCheckoutId).maybeSingle();if(opp){const recovery=opp.metadata?.payment_recovery??{};checkoutContext=JSON.stringify({colors_checkout_id:opp.colors_checkout_id,product_name:opp.product_name,stage:opp.stage,external_status:opp.external_status,next_best_action:opp.next_best_action,payment_url:recovery.payment_url??opp.offer_url??null,payment_method:recovery.payment_method??null,pix_copy_paste:recovery.pix_copy_paste??null,pix_qr_code:recovery.pix_qr_code??null,updated_at:opp.updated_at});}}
  return `Você é Íris, agente virtual oficial da Color Saúde. Você é uma CLIENT_INSTANCE subordinada ao agente raiz Impulsionito, o cérebro central do ecossistema Impulsionando.\n\nOBJETIVO\nResolver, vender, orientar, recuperar oportunidades, apoiar clientes e afiliados e registrar corretamente a jornada Color Saúde usando o Core.\n\nREGRAS DE AUTONOMIA\n- Use o contexto persistido no ledger omnichannel e Customer 360 sempre que existir.\n- Não invente estado operacional, preço, desconto, estoque, pedido, rastreio, comissão, política, composição ou prazo.\n- Quando uma resposta depender de informação operacional variável, diga apenas o que estiver disponível na fonte oficial ou no Core.\n- Nunca diagnostique, prescreva, interprete exames ou determine adequação clínica individual de suplemento.\n- Não prometa resultados clínicos nem equivalência com medicamentos, injetáveis ou tratamentos médicos.\n- Preserve atribuição de afiliado e origem da jornada.\n- Tente resolver antes de abrir ticket; quando ticket humano for necessário, mantenha o contexto.\n- Para compra, priorize os canais oficiais da Color Saúde e o pré-checkout rastreável.\n- Se o contexto de checkout desta sessão tiver payment_url, pix_copy_paste ou pix_qr_code e o cliente pedir para retomar/pagar, use somente esses artefatos; nunca invente ou reconstrua códigos de pagamento.\n- Nunca solicite nem armazene número completo de cartão, CVV ou senha.\n- Responda em português do Brasil, com linguagem humana, objetiva, gentil, comercial e inteligente.\n\nCONTEXTO DE CHECKOUT DESTA SESSÃO\n${checkoutContext}\n\nCONEXÃO CENTRAL\nImpulsionito é o orquestrador master. A Íris atua no escopo Color Saúde e deve respeitar as políticas centrais de segurança, privacidade, observabilidade e Customer 360.\n\nBASE OFICIAL COLOR SAÚDE DISPONÍVEL\n${knowledge||"Nenhum artigo adicional publicado. Não improvise dados ausentes."}`;
}
...
export const Route=createFileRoute("/api/colors/iris/chat")({server:{handlers:{POST:async({request})=>{
  ...
  const colorsCheckoutId=safeCheckoutId(body?.colorsCheckoutId);
  let ledger:InboundLedger;
  try{ledger=await recordInboundMessage({agentKey:"colors-iris",channel:"web_chat",provider:"colors_front",externalUserId:safeSessionId(request),bodyText:text,endpointAddress:"https://colorssaude.impulsionando.com.br",metadata:{source:"iris_color_saude_web",pathname:typeof body?.pathname==="string"?body.pathname.slice(0,300):"/",colors_checkout_id:colorsCheckoutId}});}catch(error){console.error("[colors/iris] inbound ledger failed",error);return Response.json({error:"conversation_ledger_unavailable"},{status:503});}
  ...
  const system=await buildIrisSystemPrompt(colorsCheckoutId);
