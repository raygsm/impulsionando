...
export const Route=createFileRoute("/api/colors/iris/chat")({server:{handlers:{POST:async({request})=>{
  ...
  try{ledger=await recordInboundMessage({agentKey:"colors-iris",channel:"web_chat",provider:"colors_front",externalUserId:safeSessionId(request),bodyText:text,endpointAddress:"https://colors.impulsionando.com.br",metadata:{source:"iris_colors_web",pathname:typeof body?.pathname==="string"?body.pathname.slice(0,300):"/"}});}catch(error){console.error("[colors/iris] inbound ledger failed",error);return Response.json({error:"conversation_ledger_unavailable"},{status:503});}
  ...
  let resolved; try{resolved=resolveProvider({});}catch{return Response.json({error:"provider_unavailable"},{status:503});}
