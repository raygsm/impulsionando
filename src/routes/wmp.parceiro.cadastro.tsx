...
function WmpParceiroCadastro() {
  ...
  useEffect(()=>{
    ...
    Promise.all([
      ...
      setCategories(categoryOptions); setStates((uf.options??[]) as RefOption[]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if(isDj&&(!form.razao_social.trim()||!form.cnpj.trim())){setError('Para DJ parceiro, razão social e CNPJ são obrigatórios.');return;}
    setSubmitting(true); setError(null);
    try {
      const res = await submitWmpParceiro({ data: {
        ...
        experiencia_anos:Number(form.experiencia_anos)||null,razao_social:form.razao_social,cnpj:form.cnpj,bio:form.bio,
        portfolio_links:links.filter((l)=>l.trim()),user_agent:typeof navigator!=="undefined"?navigator.userAgent:undefined,origem:"site_wmp_parceiro",
      }});
      navigate({to:"/wmp/obrigado/$tipo",params:{tipo:"parceiro"},search:{id:res.id}});
    } catch(err:any){setError(err?.message??"Falha ao enviar cadastro.");}
    finally{setSubmitting(false);}
  }

  return <WmpShell>
    <section className="wmp-stage-bg"><div className="mx-auto max-w-3xl px-6 pt-14 pb-10 text-center"><h1 className="wmp-display text-3xl md:text-5xl mb-3">Cadastro de Parceiro</h1><p className="opacity-80">Compartilhe seu perfil profissional. O Milito organiza a entrada e a equipe WMP faz a análise.</p></div></section>
    <section className="mx-auto max-w-3xl px-6 pb-24 -mt-6"><form onSubmit={handleSubmit} className="wmp-surface p-6 md:p-10 space-y-5">
      <div className="grid md:grid-cols-2 gap-4">
        ...
        {isDj&&<><Field label="Razão social *"><input required value={form.razao_social} onChange={e=>up("razao_social",e.target.value)} placeholder="Empresa responsável pela emissão da NF"/></Field><Field label="CNPJ *"><input required value={form.cnpj} onChange={e=>up("cnpj",maskCNPJ(e.target.value))} placeholder="CNPJ da empresa do DJ"/></Field></>}
        {!isDj&&<><Field label="Razão social (se houver)"><input value={form.razao_social} onChange={e=>up("razao_social",e.target.value)}/></Field><Field label="CNPJ (se houver)"><input value={form.cnpj} onChange={e=>up("cnpj",maskCNPJ(e.target.value))}/></Field></>}
        ...
        {isDj&&<div className="md:col-span-2 rounded-lg border p-4 text-sm"><strong>Regra financeira para DJ parceiro</strong><p className="mt-1 opacity-75">A contratação exige pessoa jurídica e emissão de nota fiscal. O pagamento operacional é previsto em até 10 dias úteis após o evento, condicionado à execução e à documentação exigida pela WMP.</p></div>}
        ...
        <div className="md:col-span-2 space-y-2"><span className="block text-sm" style={{color:"var(--wmp-muted)"}}>Portfólio (Instagram, YouTube, SoundCloud, site…)</span>{links.map((l,i)=><div key={i} className="flex gap-2"><input type="url" value={l} onChange={e=>setLinks(arr=>arr.map((x,j)=>j===i?e.target.value:x))} placeholder="https://…"/>{links.length>1&&<button type="button" onClick={()=>setLinks(arr=>arr.filter((_,j)=>j!==i))} className="wmp-cta wmp-cta-outline" style={{padding:"0.5rem"}}><Trash2 className="size-4"/></button>}</div>)}<button type="button" onClick={()=>setLinks(arr=>[...arr,""])} className="wmp-cta wmp-cta-outline" style={{padding:"0.5rem 1rem",fontSize:"0.875rem"}}><Plus className="size-4"/> Adicionar link</button></div>
      </div>
...
function Field({label,children,full}:{label:string;children:React.ReactNode;full?:boolean}){return <label className={`block space-y-1.5 ${full?"md:col-span-2":""}`}><span className="block text-sm" style={{color:"var(--wmp-muted)"}}>{label}</span>{children}</label>;}
