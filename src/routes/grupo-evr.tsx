...
import { Activity, BarChart3, CalendarDays, Pill, ShieldCheck } from "lucide-react";
    ...
    <div data-tenant="grupo-evr" className="min-h-screen bg-[#f7f8f6] text-[#12231d]">
      <header className="sticky top-0 z-40 border-b border-[#dbe2dc] bg-white/95 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-5 px-5 py-4 lg:px-8">
          ...
          <nav className="hidden items-center gap-6 text-sm font-medium md:flex">
            <a href="/grupo-evr#grupo">O grupo</a>
            <a href="/grupo-evr#autoridades">Autoridades</a>
            <a href="/grupo-evr#empresas">Empresas</a>
      ...
      <footer className="border-t border-[#dbe2dc] bg-[#12231d] text-white">
        <div className="mx-auto grid max-w-7xl gap-8 px-5 py-10 md:grid-cols-3 lg:px-8">
          <div><p className="font-semibold tracking-[0.12em]">GRUPO EVR</p><p className="mt-2 max-w-sm text-sm leading-6 text-white/70">Ecossistema integrado de saúde, atendimento, serviços e varejo farmacêutico com gestão orientada por dados.</p></div>
          <div className="grid grid-cols-2 gap-3 text-sm text-white/80"><span className="flex items-center gap-2"><CalendarDays className="h-4 w-4" /> Agenda inteligente</span><span className="flex items-center gap-2"><Pill className="h-4 w-4" /> Ativese Pharma</span><span className="flex items-center gap-2"><BarChart3 className="h-4 w-4" /> BI executivo</span><span className="flex items-center gap-2"><ShieldCheck className="h-4 w-4" /> Governança</span></div>
