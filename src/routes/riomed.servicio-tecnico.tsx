...
const SERVICES = [
  ...
  { icon: GraduationCap, t: "Capacitación", d: "Entrenamiento del equipo clínico para uso seguro del equipo." },
    ...
    <div className="max-w-6xl mx-auto px-4 py-12 space-y-14">
      <header>
        <nav aria-label="Migas de pan" className="text-xs text-slate-500 mb-3">
          ...
          <span className="text-slate-700 font-medium">Servicio técnico</span>
        </nav>
        ...
        <div className="flex flex-wrap gap-3 mt-5">
          ...
          <Link to="/riomed/vendedor" className="rounded-full px-6 py-3 border font-semibold">Hablar con vendedor</Link>
          <Link to="/riomed/tecnico/cadastro" className="rounded-full px-6 py-3 border font-semibold">Soy técnico</Link>
        </div>
      </header>
      ...
      <section className="rounded-2xl border bg-slate-50 p-8">
        ...
        <ol className="grid md:grid-cols-4 gap-4 mt-4 text-sm">
          {["Abrí tu ticket","Diagnóstico técnico","Presupuesto y aprobación","Servicio + reporte"].map((s, i) => (
