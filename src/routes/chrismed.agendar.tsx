 ...
 * é a confirmação definitiva. O checkout mantém apenas um hold temporário de 90s.
 */
...
const CHECKOUT_HOLD_SECONDS = 90;
...
export const Route = createFileRoute('/chrismed/agendar')({
  ...
  errorComponent: ({ error, reset }) => <div className="container py-12 text-center"><h1 className="text-2xl font-semibold mb-2">Ops, algo deu errado</h1><p className="text-muted-foreground mb-4">{String(error)}</p><Button onClick={reset}>Tentar novamente</Button></div>,
...
function ChrismedAgendarPage() {
  ...
  function applyCare360(mod: 'telemedicina' | 'domiciliar') {
    ...
    setSpecialty(CARE_360); if (doc) setDoctor(doc); setModality(mod); if (targetUnit) setUnit(targetUnit); setStep('schedule');
  }
  ...
  const currentOffering = useMemo(() => {
    ...
    if (modality === 'presencial') {
      if (specialty?.slug === 'hepatologia') return offerings.find((o) => o.slug === 'consulta-presencial-hepatologia') ?? null;
      if (specialty?.slug === 'gastroenterologia' || specialty?.slug === 'clinica-medica') return offerings.find((o) => o.slug === 'consulta-presencial-gastro-clinica') ?? null;
      return null;
    }
    if (modality === 'telemedicina') return offerings.find((o) => o.slug === 'telemedicina') ?? null;
    if (modality === 'domiciliar') return offerings.find((o) => o.slug === 'visita-domiciliar') ?? null;
    return offerings.find((o) => o.modality === modality && o.slug !== 'homologacao-mercado-pago-1-real') ?? null;
  }, [modality, specialty?.slug, offerings]);
  ...
  useEffect(() => { setSelectedDayIso(null); setSelectedTime(null); setSelectedSlotId(null); setMonthOffset(0); }, [modality, specialty?.slug]);
  ...
  async function handlePay() {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('mpago-create-payment', { body: {
        ...
        payer: { email: patient.email, first_name: patient.first_name, last_name: patient.last_name || undefined, identification: patient.doc ? { type: 'CPF', number: patient.doc.replace(/\D/g, '') } : undefined },
      }});
    ...
    } catch (e) { toast.error(`Erro ao gerar PIX: ${(e as Error).message}`); }
  ...
  return <ChrismedShell variant="minimal"><div className="mx-auto w-full max-w-5xl px-4 md:px-6 py-10 pb-32 sm:pb-10">
    ...
    {step === 'specialty' && (() => { const base = doctor ? CHRISMED_SPECIALTIES.filter((s) => doctor.specialtySlugs.includes(s.slug)) : CHRISMED_SPECIALTIES; const specialtiesToShow = base.filter((s) => ['gastroenterologia','hepatologia','clinica-medica'].includes(s.slug)); return <section aria-labelledby="s2"><button onClick={() => setStep('modality')} className="text-sm text-[var(--chrismed-ink)] hover:underline mb-3">← Trocar modalidade</button><h2 id="s2" className="chrismed-serif text-3xl md:text-4xl text-[var(--chrismed-ink)]">Escolha a especialidade</h2><p className="mt-2 text-[var(--chrismed-graphite)]">Consulta presencial no consultório em Copacabana. A escolha é obrigatória antes do pagamento porque Hepatologia possui duração e valor diferentes.</p><div className="mt-8 grid sm:grid-cols-2 lg:grid-cols-3 gap-4">{specialtiesToShow.map((sp) => { const Icon = SPECIALTY_ICON[sp.icon]; const isHepato = sp.slug === 'hepatologia'; return <button key={sp.slug} type="button" onClick={() => { setSpecialty(sp); setStep('doctor'); }} className="text-left rounded-xl border border-[var(--chrismed-sand)] bg-[var(--chrismed-ivory)] p-5 hover:border-[var(--chrismed-champagne-deep)]"><div className="h-11 w-11 rounded-lg bg-[var(--chrismed-bone)] text-[var(--chrismed-ink)] flex items-center justify-center mb-3"><Icon className="h-5 w-5" /></div><div className="chrismed-serif text-lg text-[var(--chrismed-ink)]">{sp.name}</div><div className="text-sm text-[var(--chrismed-graphite)] mt-1">{sp.short}</div><div className="mt-3 font-semibold text-[var(--chrismed-ink)]">{isHepato ? 'R$ 1.200 · 60 minutos' : 'R$ 1.000 · 45 minutos'}</div></button>; })}</div><div className="mt-6 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-950"><strong>Atenção:</strong> se você já tem pré-diagnóstico, encaminhamento ou investigação relacionada ao fígado, escolha <strong>Hepatologia</strong>. Em caso de dúvida, fale com Oliver antes de avançar.</div></section>; })()}
