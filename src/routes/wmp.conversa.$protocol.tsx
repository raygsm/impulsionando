...
export const Route = createFileRoute('/wmp/conversa/$protocol')({
  ...
  head: () => ({ meta: [{ title: 'Exportar conversa com o Milito — WMP' }, { name: 'robots', content: 'noindex, nofollow' }] }),
...
function WmpConversationExportPage() {
  ...
  const requiredReady = useMemo(
    () => form.fullName.trim().length >= 5 && isValidEmail(form.email) && isValidPhoneBR(form.phone),
    [form.fullName, form.email, form.phone],
  )

  useEffect(() => {
    if (!tokenLooksValid) { setInvalid(true); setLoading(false); return }
    void (async () => {
      ...
      if (stateError || !data) { setInvalid(true); setLoading(false); return }
      const state = data as unknown as Record<string, any>
      if (state.valid !== true) { setInvalid(true); setLoading(false); return }
      if (state.registered) {
        setForm((current) => ({
          ...current,
  ...
  async function fillCep() {
    ...
    try {
      ...
      if (!result) return
      setForm((current) => ({ ...current, cep: maskCEP(result.cep), logradouro: result.logradouro, bairro: result.bairro, cidade: result.cidade, estado: result.uf }))
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : 'Não foi possível enviar a conversa. Tente novamente.')
    } finally { setSubmitting(false) }
  }

  if (loading) return <Frame><div className="flex min-h-[52vh] items-center justify-center"><Loader2 className="h-7 w-7 animate-spin text-[#6f3c87]" /></div></Frame>
  if (invalid) return <Frame><Card className="mx-auto max-w-xl border-[#e2d5e8] shadow-sm"><CardContent className="space-y-4 py-10 text-center"><ShieldCheck className="mx-auto h-9 w-9 text-[#6f3c87]" /><h1 className="text-xl font-semibold">Link de atendimento inválido ou expirado</h1><p className="text-sm text-muted-foreground">Volte ao atendimento do Milito e solicite novamente a exportação da conversa.</p><Button asChild><Link to="/wmp">Falar com o Milito</Link></Button></CardContent></Card></Frame>
  if (sent) return <Frame><Card className="mx-auto max-w-xl border-[#e2d5e8] shadow-sm"><CardContent className="space-y-5 py-10 text-center"><CheckCircle2 className="mx-auto h-11 w-11 text-emerald-600" /><div><h1 className="text-2xl font-semibold">Conversa enviada</h1><p className="mt-2 text-sm text-muted-foreground">A íntegra do atendimento foi enviada ao e-mail cadastrado.</p></div><div className="rounded-xl bg-[#f7f1fa] px-4 py-3 text-sm"><span className="text-muted-foreground">Protocolo</span><div className="mt-1 font-mono font-semibold text-[#4d2760]">{protocol}</div></div><Button asChild variant="outline"><Link to="/wmp">Voltar para a WMP</Link></Button></CardContent></Card></Frame>

  return (
    <Frame>
      <div className="mx-auto grid max-w-5xl gap-6 lg:grid-cols-[0.9fr_1.6fr]">
        <aside className="space-y-4">
          <div><p className="text-sm font-semibold uppercase tracking-[0.16em] text-[#80539a]">Exportação segura</p><h1 className="mt-2 text-3xl font-bold tracking-tight">Receba sua conversa com o Milito</h1><p className="mt-3 text-sm leading-6 text-muted-foreground">Para enviar a conversa e o protocolo por e-mail, precisamos apenas de nome completo, celular e e-mail. Os demais dados são opcionais e ajudam a WMP a conhecer melhor seu contexto nas próximas interações.</p></div>
        ...
        <Card className="border-[#e2d5e8] shadow-sm">
          ...
          <CardContent><form onSubmit={submit} className="space-y-6">
            <div className="grid gap-4 md:grid-cols-2"><Field label="Nome completo *" className="md:col-span-2"><Input value={form.fullName} onChange={(e) => setForm((f) => ({ ...f, fullName: e.target.value }))} autoComplete="name" required /></Field><Field label="Celular / WhatsApp *"><Input value={form.phone} onChange={(e) => setForm((f) => ({ ...f, phone: maskPhone(e.target.value) }))} inputMode="tel" autoComplete="tel" required /></Field><Field label="E-mail *"><Input value={form.email} onChange={(e) => setForm((f) => ({ ...f, email: e.target.value }))} type="email" autoComplete="email" required /></Field></div>
            <button type="button" onClick={() => setOptionalOpen((value) => !value)} className="flex w-full items-center justify-between rounded-xl border border-[#e2d5e8] bg-[#faf7fb] px-4 py-3 text-left"><div><p className="font-semibold">Completar cadastro opcional</p><p className="mt-0.5 text-xs text-muted-foreground">CPF, endereço e informações da empresa podem ser preenchidos agora ou depois.</p></div><ChevronDown className={`h-5 w-5 transition ${optionalOpen ? 'rotate-180' : ''}`} /></button>
            {optionalOpen ? <div className="space-y-4 rounded-2xl border border-[#e2d5e8] p-4"><div className="grid gap-4 md:grid-cols-2"><Field label="CPF"><Input value={form.cpf} onChange={(e) => setForm((f) => ({ ...f, cpf: maskCPF(e.target.value) }))} inputMode="numeric" /></Field><Field label="CEP"><div className="flex gap-2"><Input value={form.cep} onChange={(e) => setForm((f) => ({ ...f, cep: maskCEP(e.target.value) }))} onBlur={fillCep} inputMode="numeric" /><Button type="button" variant="outline" onClick={fillCep} disabled={cepLoading || !isValidCEP(form.cep)}>{cepLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Buscar'}</Button></div></Field><Field label="Endereço" className="md:col-span-2"><Input value={form.logradouro} onChange={(e) => setForm((f) => ({ ...f, logradouro: e.target.value }))} /></Field><Field label="Número"><Input value={form.numero} onChange={(e) => setForm((f) => ({ ...f, numero: e.target.value }))} /></Field><Field label="Complemento"><Input value={form.complemento} onChange={(e) => setForm((f) => ({ ...f, complemento: e.target.value }))} /></Field><Field label="Bairro"><Input value={form.bairro} onChange={(e) => setForm((f) => ({ ...f, bairro: e.target.value }))} /></Field><Field label="Cidade"><Input value={form.cidade} onChange={(e) => setForm((f) => ({ ...f, cidade: e.target.value }))} /></Field><Field label="Estado"><Input value={form.estado} maxLength={2} onChange={(e) => setForm((f) => ({ ...f, estado: e.target.value.toUpperCase().slice(0, 2) }))} /></Field></div><label className="flex cursor-pointer items-center gap-3 rounded-xl bg-[#faf7fb] p-3"><Checkbox checked={form.isCompany} onCheckedChange={(checked) => setForm((f) => ({ ...f, isCompany: checked === true }))} /><span className="text-sm font-medium">Estou falando em nome de uma empresa</span></label>{form.isCompany ? <div className="grid gap-4 md:grid-cols-2"><Field label="Razão social / nome da empresa"><Input value={form.companyName} onChange={(e) => setForm((f) => ({ ...f, companyName: e.target.value }))} /></Field><Field label="CNPJ"><Input value={form.companyDocument} onChange={(e) => setForm((f) => ({ ...f, companyDocument: maskCNPJ(e.target.value) }))} inputMode="numeric" /></Field></div> : null}</div> : null}
            ...
            <p className="text-center text-xs leading-5 text-muted-foreground">Ao solicitar o envio, você autoriza a WMP a usar o e-mail informado para esta comunicação e para o relacionamento decorrente deste atendimento, conforme as políticas aplicáveis.</p>
          </form></CardContent>
        </Card>
      </div>
    </Frame>
