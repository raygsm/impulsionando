...
import { ArrowRight, BarChart3, Bot, BriefcaseBusiness, CalendarDays, CheckCircle2, Coffee, Mail, MessageCircle, ShoppingBasket, Sparkles, Users } from 'lucide-react';

export const Route = createFileRoute('/gruposmart')({ component: GrupoSmartHome });

const modules = [
  ['CRM Full', 'Lead único, origem, vertical, ICP, score, owner, SLA e histórico.'],
  ['Inside + Field Sales', 'Hunters qualificam; executivos regionais visitam, negociam e fecham.'],
  ['Agenda inteligente', 'Reuniões online, visitas, bloqueios, lembretes, no-show e handoff.'],
  ['Jornadas N8N', 'Cadências por público, estágio, origem, vertical, região e comportamento.'],
  ['WhatsApp + e-mail', 'Templates, consentimento, rastreio e comunicação transacional/comercial.'],
  ['BI comercial', 'Metas, conversão, aging, produtividade, forecast, origem e cross-sell.'],
];
...
function GrupoSmartHome() {
  return <main className="min-h-screen bg-slate-950 text-white" data-tenant="gruposmart">
    ...
    <section className="relative overflow-hidden px-6 py-24 md:py-32">
      ...
      <div className="relative mx-auto grid max-w-7xl gap-12 lg:grid-cols-[1.15fr_.85fr] lg:items-center">
        <div><div className="mb-5 inline-flex items-center gap-2 rounded-full border border-sky-400/30 bg-sky-400/10 px-4 py-2 text-xs font-bold uppercase tracking-[.18em] text-sky-300"><Sparkles size={15}/> Conveniência corporativa conectada</div>
          ...
          <p className="mt-7 max-w-2xl text-lg leading-8 text-slate-300">wizMart e Smart Café preservam seus produtos, públicos e jornadas. O Grupo Smart conecta dados, oportunidades, equipe e relacionamento em uma única estrutura preparada para crescer.</p>
          <div className="mt-9 flex flex-wrap gap-3"><a href="#solucoes" className="inline-flex items-center gap-2 rounded-xl bg-sky-500 px-6 py-3 font-bold text-slate-950">Conhecer soluções <ArrowRight size={18}/></a><a href="/gruposmart/app" className="rounded-xl border border-white/20 px-6 py-3 font-semibold">Entrar no cockpit Full</a></div>
        </div>
    ...
    <section id="operacao" className="border-y border-white/10 bg-white/[.035] px-6 py-20"><div className="mx-auto max-w-7xl"><p className="text-sm font-bold uppercase tracking-[.2em] text-sky-400">Nova arquitetura comercial</p><h2 className="mt-3 text-4xl font-black md:text-5xl">Da prospecção ao fechamento, sem perder contexto.</h2><div className="mt-10 grid gap-5 md:grid-cols-3">
      {[['Hunters internos',Users,'Prospecção ativa + inbound, qualificação, ICP, cadência e reunião.'],['Executivos de campo',BriefcaseBusiness,'Atuação regional, visita, diagnóstico, proposta, negociação e fechamento.'],['Smartito',Bot,'Agente especializado do Impulsionito: identifica intenção, vertical, região e próxima melhor ação.']].map(([t,I,d]:any)=><article key={t} className="rounded-2xl border border-white/10 bg-slate-900 p-6"><I className="text-sky-400"/><h3 className="mt-5 text-xl font-bold">{t}</h3><p className="mt-2 leading-7 text-slate-400">{d}</p></article>)}
    </div></div></section>

    <section id="full" className="px-6 py-20"><div className="mx-auto max-w-7xl"><div className="max-w-3xl"><p className="text-sm font-bold uppercase tracking-[.2em] text-sky-400">Impulsionando Tecnologia · Plano Full</p><h2 className="mt-3 text-4xl font-black md:text-5xl">A tecnologia deixa de ser projeto e vira infraestrutura.</h2><p className="mt-5 text-lg leading-8 text-slate-300">A assessoria comercial e de marketing liderada por Priscila Caldas define pessoas, metas, modelo de trabalho e processo. O Plano Full materializa a estratégia em CRM, jornadas, automações e gestão.</p></div><div className="mt-10 grid gap-4 md:grid-cols-2 lg:grid-cols-3">{modules.map(([t,d],i)=>{const Icons=[BarChart3,Users,CalendarDays,Sparkles,MessageCircle,Mail];const I=Icons[i];return <article key={t} className="rounded-2xl border border-white/10 bg-white/[.04] p-6"><I className="text-sky-400"/><h3 className="mt-4 font-bold">{t}</h3><p className="mt-2 text-sm leading-6 text-slate-400">{d}</p></article>})}</div></div></section>
