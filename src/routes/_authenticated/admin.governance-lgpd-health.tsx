  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm">Governance Apps</CardTitle></CardHeader><CardContent><div className="text-2xl font-bold">{fmt(data.governance.pending)}<span className="text-sm text-muted-foreground">/{fmt(data.governance.total)}</span></div><p className="text-xs text-muted-foreground">{fmt(data.governance.approved)} aprovados</p></CardContent></Card>
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm">Compliance</CardTitle></CardHeader><CardContent><div className="text-2xl font-bold">{fmt(data.compliance.ok)}<span className="text-sm text-muted-foreground">/{fmt(data.compliance.total)}</span></div><p className="text-xs text-muted-foreground">{fmt(data.compliance.gaps)} gaps</p></CardContent></Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card><CardHeader><CardTitle className="text-sm">Audit por Severidade</CardTitle></CardHeader><CardContent>
          <table className="w-full text-sm"><tbody>{data.audit.bySeverity.map((r:any)=>(<tr key={r.severity} className="border-t"><td className="py-1">{r.severity}</td><td className="text-right">{fmt(r.count)}</td></tr>))}</tbody></table>
        </CardContent></Card>
