...
import { Building2, CalendarCheck2, FileText, Mail, RefreshCw } from 'lucide-react'
...
function WmpOperationsPage(){
  ...
  const operationalOpen=(data.events??[]).filter((row:any)=>!['COMPLETED','CANCELLED'].includes(row.status))

  return <main className="mx-auto max-w-7xl space-y-6 p-6 md:p-8">
    <header className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between"><div><p className="text-sm text-muted-foreground">WMP — Wagner Miller Produções</p><h1 className="text-3xl font-semibold tracking-tight">Central operacional</h1><p className="mt-1 text-sm text-muted-foreground">CRM, eventos, agenda corporativa, bookings de DJs, equipamentos, repasses e atendimento em dados reais.</p></div><button type="button" onClick={()=>void load()} className="inline-flex items-center gap-2 rounded-md border px-4 py-2 text-sm"><RefreshCw className="size-4"/>Atualizar</button></header>

    <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">{[
      ['Briefings',data.briefings?.length],['Datas corporativas',data.corporateDates?.length],['Eventos operacionais',data.events?.length],['Parceiros',data.partners?.length],['Bookings',data.bookings?.length],['Equipamentos',data.equipment?.length],['Locações',data.rentals?.length],['Repasses',data.payouts?.length],['Tickets Milito',data.tickets?.length],['Eventos em aberto',operationalOpen.length],
    ...
    {loading?<div className="rounded-xl border p-10 text-sm text-muted-foreground">Carregando operação...</div>:<>
      ...
      {tab==='agenda'&&<section className="space-y-5">
        <div className="rounded-xl border bg-card"><div className="border-b p-4"><div className="flex items-center gap-2"><Building2 className="size-4"/><h2 className="font-semibold">Agenda corporativa solicitada</h2></div><p className="mt-1 text-xs text-muted-foreground">Datas captadas por hotéis e empresas. Cada data evolui separadamente para cotação, confirmação ou cancelamento sem duplicar o lead-mãe.</p></div><div className="overflow-x-auto"><table className="w-full min-w-[1180px] text-sm"><thead><tr className="border-b text-left text-xs text-muted-foreground"><th className="p-3">Empresa / contato</th><th>Data</th><th>Local</th><th>Horário</th><th>Observação</th><th>Ação</th><th>Status</th></tr></thead><tbody>{(data.corporateDates??[]).length===0?<tr><td colSpan={7} className="p-6 text-center text-muted-foreground">Nenhuma data corporativa solicitada.</td></tr>:(data.corporateDates??[]).map(r=>{const briefing=briefingsById.get(r.briefing_id) as any;return <tr key={r.id} className="border-b last:border-0"><td className="p-3"><b>{briefing?.contratante_empresa||briefing?.contratante_nome||'Demanda corporativa'}</b><div className="text-xs text-muted-foreground">{briefing?.contratante_nome||'—'} · {briefing?.contratante_email||'—'}</div></td><td>{date(r.event_date)}</td><td><b>{r.venue_name||'Local a definir'}</b><div className="text-xs text-muted-foreground">{[r.venue_bairro,r.venue_city,r.venue_state].filter(Boolean).join(' · ')||'—'}</div></td><td>{time(r.start_time)} → {time(r.end_time)}</td><td className="max-w-[260px] text-xs text-muted-foreground">{r.notes||'—'}</td><td><a href={`/wmp/propostas/nova?briefing_date_id=${encodeURIComponent(r.id)}`} className="inline-flex items-center gap-1.5 rounded border bg-background px-2 py-1 text-[11px] hover:bg-muted"><FileText className="size-3"/>{r.status==='QUOTED'?'Nova versão/cotação':'Criar proposta'}</a></td><td><StatusButtons table="wmp_briefing_dates" id={r.id} current={r.status} options={['REQUESTED','QUOTED','CONFIRMED','CANCELLED']} onDone={()=>void load()}/></td></tr>})}</tbody></table></div></div>

        <div className="rounded-xl border bg-card"><div className="border-b p-4"><div className="flex items-center gap-2"><CalendarCheck2 className="size-4"/><h2 className="font-semibold">Eventos operacionais WMP</h2></div><p className="mt-1 text-xs text-muted-foreground">Após o aceite da proposta e a assinatura do contrato, o evento operacional é criado e confirmado automaticamente. A conclusão do evento dispara a jornada pós-evento quando a automação estiver homologada.</p></div><div className="overflow-x-auto"><table className="w-full min-w-[1050px] text-sm"><thead><tr className="border-b text-left text-xs text-muted-foreground"><th className="p-3">Evento</th><th>Data</th><th>Local</th><th>Origem</th><th>Publicação</th><th>Status operacional</th></tr></thead><tbody>{(data.events??[]).length===0?<tr><td colSpan={6} className="p-6 text-center text-muted-foreground">Nenhum evento operacional registrado.</td></tr>:(data.events??[]).map(r=><tr key={r.id} className="border-b last:border-0"><td className="p-3"><b>{r.title}</b><div className="text-xs text-muted-foreground">{r.event_type||'Tipo não informado'}</div></td><td>{date(r.event_date)}<div className="text-xs text-muted-foreground">{time(r.start_time)} → {time(r.end_time)}</div></td><td><b>{r.venue_name||'Local a definir'}</b><div className="text-xs text-muted-foreground">{[r.venue_bairro,r.venue_city,r.venue_state].filter(Boolean).join(' · ')||'—'}</div></td><td>{r.source}</td><td>{r.public_status}</td><td><StatusButtons table="wmp_events" id={r.id} current={r.status} options={['PLANNED','CONFIRMED','IN_PROGRESS','COMPLETED','CANCELLED']} onDone={()=>void load()}/></td></tr>)}</tbody></table></div></div>
