...
export const Route = createFileRoute('/_authenticated/chrismed/eventos')({
...
function ChrismedEventsDashboard() {
  ...
  async function loadEvents() {
    ...
    if (error) {
      toast.error('Não foi possível carregar os eventos autorizados.');
      setEvents([]);
      setLoading(false);
      return;
    }
  ...
  async function loadOverview(eventId: string) {
    ...
    if (error) {
      toast.error('Não foi possível carregar os indicadores deste evento.');
      setOverview(null);
    } else {
  ...
  const conversion = useMemo(() => {
    const invited = overview?.metrics.invited ?? 0;
    const confirmed = overview?.metrics.confirmed ?? 0;
    return invited > 0 ? Math.round((confirmed / invited) * 100) : 0;
  }, [overview]);
  const attendance = useMemo(() => {
    const confirmed = overview?.metrics.confirmed ?? 0;
    const checked = overview?.metrics.checked_in ?? 0;
    return confirmed > 0 ? Math.round((checked / confirmed) * 100) : 0;
  }, [overview]);
  const surveyRate = useMemo(() => {
    const scheduled = overview?.metrics.surveys_scheduled ?? 0;
    const completed = overview?.metrics.surveys_completed ?? 0;
    ...
    <main className="min-h-screen bg-[#F7F3EA] px-4 py-7 text-[#071C18] sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl space-y-6">
        <header className="rounded-3xl border border-[#D9D3CB] bg-white p-6 shadow-sm lg:flex lg:items-center lg:justify-between lg:gap-6">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#596660]">Eventos CHRISMED</p>
            <h1 className="mt-2 text-3xl font-bold tracking-tight">Dashboard B2B de eventos</h1>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-[#3F4A47]">Acompanhe convites, confirmações, check-ins, comparecimento e pesquisas. Organizações vinculadas enxergam somente os eventos para os quais receberam permissão.</p>
          </div>
          <div className="mt-5 flex min-w-[280px] items-end gap-2 lg:mt-0">
            <div className="flex-1"><Select value={selected} onValueChange={(value) => { setSelected(value); void loadOverview(value); }}><SelectTrigger><SelectValue placeholder="Selecione um evento" /></SelectTrigger><SelectContent>{events.map((event) => <SelectItem key={event.event_id} value={event.event_id}>{event.title}</SelectItem>)}</SelectContent></Select></div>
            <Button variant="outline" onClick={() => void loadEvents()} disabled={loading}><RefreshCw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`} /></Button>
          </div>
        </header>
        ...
        {loading ? <div className="flex min-h-48 items-center justify-center gap-3 rounded-2xl border border-[#D9D3CB] bg-white text-sm text-[#596660]"><Loader2 className="h-5 w-5 animate-spin" />Carregando indicadores…</div> : !overview ? <div className="rounded-2xl border border-[#D9D3CB] bg-white p-10 text-center text-sm text-[#596660]">Nenhum evento disponível para este usuário.</div> : (
          <>
            ...
            <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
              <Metric title="Convidados" value={overview.metrics.invited} sub={`${overview.metrics.invitation_accepted} aceitaram convite`} icon={Users} />
              <Metric title="Confirmados" value={overview.metrics.confirmed} sub={`${conversion}% dos convidados`} icon={CheckCircle2} />
              <Metric title="Check-ins" value={overview.metrics.checked_in} sub={`${attendance}% de comparecimento`} icon={CalendarDays} />
              <Metric title="Pesquisas respondidas" value={overview.metrics.surveys_completed} sub={`${surveyRate}% de resposta`} icon={BarChart3} />
            </section>

            <section className="grid gap-4 lg:grid-cols-2">
              <Card className="border-[#D9D3CB] bg-white"><CardHeader><CardTitle>Satisfação</CardTitle></CardHeader><CardContent className="grid grid-cols-2 gap-4"><Score label="NPS médio" value={overview.metrics.avg_nps} max="10" /><Score label="CSAT médio" value={overview.metrics.avg_csat} max="5" /></CardContent></Card>
              <Card className="border-[#D9D3CB] bg-white"><CardHeader><CardTitle>Funil do evento</CardTitle></CardHeader><CardContent className="space-y-3 text-sm">{[['Convidados',overview.metrics.invited],['Inscrições',overview.metrics.registrations],['Confirmados',overview.metrics.confirmed],['Check-ins',overview.metrics.checked_in],['Pesquisas',overview.metrics.surveys_completed]].map(([label,value]) => <div key={String(label)} className="flex items-center justify-between border-b border-[#E7E2DB] pb-2"><span className="text-[#596660]">{label}</span><strong>{value}</strong></div>)}</CardContent></Card>
            </section>
          </>
        )}
      </div>
...
function Metric({ title, value, sub, icon: Icon }: { title: string; value: number; sub: string; icon: typeof Users }) {
...
function Score({ label, value, max }: { label: string; value: number | null; max: string }) {
  return <div className="rounded-2xl border border-[#E7E2DB] bg-[#FDFCFB] p-5"><div className="text-xs uppercase tracking-[0.14em] text-[#596660]">{label}</div><div className="mt-2 text-3xl font-bold">{value ?? '—'}<span className="text-sm font-normal text-[#596660]">/{max}</span></div></div>;
}
