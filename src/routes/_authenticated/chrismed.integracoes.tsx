...
function ChrismedIntegrations() {
  ...
  async function syncGoogleDrive() {
    ...
    try {
      ...
      toast.success(`${body.indexed ?? 0} arquivos indexados; ${body.withText ?? 0} documentos institucionais com conteúdo textual disponível para contexto.`);
      await load();
    } catch (error) {
      toast.error((error as Error).message);
    } finally {
    ...
    <main className="min-h-screen bg-[#F7F3EA] px-4 py-7 text-[#071C18] sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
          ...
          <Metric icon={Cloud} label="Docs ativos" value={counts.indexedDocuments} />
        ...
        <section className="grid gap-5 lg:grid-cols-2">
          ...
          <Card className={oliverReady ? 'border-emerald-300 bg-white' : 'border-amber-300 bg-white'}>
            ...
            <CardContent className="space-y-4 text-sm">
              ...
              <div className="rounded-xl border bg-[#FDFCFB] p-4"><div className="flex items-center gap-2 font-medium"><ShieldCheck className="h-4 w-4" />Regra documental</div><p className="mt-2 text-xs text-[#596660]">Oliver pode localizar, ler, compreender e relacionar documentos internos. No chat público, apenas conteúdo institucional sem titular é elegível para grounding. A entrega de documento é limitada ao proprietário autenticado ou contexto autorizado.</p></div>
            </CardContent>
          </Card>
        </section>
