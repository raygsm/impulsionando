...
import { getAnaMaduWhatsAppPairing, startAnaMaduWhatsAppPairing } from '@/lib/anamadu/whatsapp-pairing.functions';
...
function WhatsAppPairingPage() {
  const statusFn = useServerFn(getAnaMaduWhatsAppPairing);
  const startFn = useServerFn(startAnaMaduWhatsAppPairing);
  const [qr, setQr] = useState<string | null>(null);
  const [pairingCode, setPairingCode] = useState<string | null>(null);

  const status = useQuery({
    queryKey: ['anamadu-whatsapp-pairing'],
    queryFn: () => statusFn(),
  ...
  return <div className="container mx-auto max-w-5xl p-6">
    <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
      <div>
        ...
        <h1 className="text-2xl font-bold">WhatsApp · Ana Madú · Annita</h1>
        <p className="mt-1 text-sm text-muted-foreground">Pareamento seguro do canal WhatsApp da Annita, instância especializada do Impulsionito.</p>
      </div>
    ...
    <div className="grid gap-4 md:grid-cols-3">
      ...
      <Card><CardHeader className="pb-2"><CardTitle className="text-sm">Provedor</CardTitle></CardHeader><CardContent><div className="font-semibold">{s?.endpointProvider || '—'}</div><p className="mt-2 text-xs text-muted-foreground">Instância: {s?.instance || 'anamadu-annita'}</p></CardContent></Card>
    ...
    <Card className="mt-5">
      <CardHeader><CardTitle className="flex items-center gap-2"><QrCode className="size-5" />Parear WhatsApp</CardTitle></CardHeader>
      <CardContent className="grid gap-6 md:grid-cols-[1fr_320px]">
        <div>
          <p className="text-sm leading-relaxed text-muted-foreground">O QR Code é gerado apenas no servidor e exibido somente nesta área autenticada. A chave do provedor nunca aparece no navegador, no QR ou no histórico da conversa.</p>
          <div className="mt-4 rounded-xl border bg-muted/30 p-4 text-sm">
            <div className="flex items-start gap-3"><ShieldCheck className="mt-0.5 size-5 shrink-0" /><div><strong>Isolamento por cliente</strong><p className="mt-1 text-muted-foreground">A Ana Madú usa credenciais próprias. Nenhum token de outro tenant é reutilizado.</p></div></div>
          </div>
          {blocked && <div className="mt-4 rounded-xl border border-amber-300 bg-amber-50 p-4 text-sm text-amber-900"><strong>Infraestrutura pronta; credencial do provedor ainda não instalada.</strong><p className="mt-1">Defina, no servidor, ANAMADU_EVOLUTION_BASE_URL e ANAMADU_EVOLUTION_API_KEY. O sistema continuará como PENDING_CONNECTION até existir uma sessão real.</p></div>}
        ...
        <div className="flex min-h-72 items-center justify-center rounded-2xl border bg-white p-4">
          {connected ? <div className="text-center"><CheckCircle2 className="mx-auto size-14 text-emerald-600" /><strong className="mt-3 block">WhatsApp conectado</strong><p className="mt-1 text-xs text-muted-foreground">A Annita pode assumir o canal após a confirmação do webhook e do teste E2E.</p></div> : qr ? <img src={qr} alt="QR Code para parear WhatsApp Ana Madú" className="max-h-64 max-w-64" /> : <div className="text-center text-muted-foreground"><QrCode className="mx-auto size-16 opacity-30" /><p className="mt-3 text-sm">O QR aparecerá aqui.</p></div>}
        </div>
      </CardContent>
    </Card>
  </div>;
