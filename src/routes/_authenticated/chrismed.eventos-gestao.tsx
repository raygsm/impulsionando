...
import { CalendarDays, CheckCircle2, Eye, Pencil, Plus, RefreshCw, Users, XCircle } from 'lucide-react';
...
function ChrismedEventManagementPage() {
  ...
  return <div className="mx-auto max-w-7xl space-y-6 p-6">
    <header className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
      <div><p className="text-sm font-medium text-primary">CHRISMED · Gestão</p><h1 className="text-3xl font-bold tracking-tight">Eventos</h1><p className="mt-1 text-sm text-muted-foreground">Crie, publique, acompanhe inscrições e encerre eventos sem acesso direto ao banco.</p></div>
      <div className="flex gap-2"><Button variant="outline" onClick={() => void load()} disabled={loading}><RefreshCw className={`mr-2 h-4 w-4 ${loading ? 'animate-spin' : ''}`} />Atualizar</Button><Button onClick={startNew}><Plus className="mr-2 h-4 w-4" />Novo evento</Button></div>
    </header>
    ...
    {editing && <Card><CardHeader><CardTitle>{form.id ? 'Editar evento' : 'Novo evento'}</CardTitle></CardHeader><CardContent className="grid gap-4 md:grid-cols-2">
      ...
      <div className="space-y-2 md:col-span-2"><Label>Descrição</Label><textarea className="min-h-28 w-full rounded-md border bg-background p-3 text-sm" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} /></div>
      ...
      <div className="flex gap-2 md:col-span-2"><Button onClick={() => void save()} disabled={saving}>{saving ? 'Salvando…' : 'Salvar evento'}</Button><Button variant="outline" onClick={() => setEditing(false)}>Cancelar</Button></div>
    </CardContent></Card>}
    ...
    <Card><CardHeader><CardTitle className="flex items-center gap-2"><CalendarDays className="h-5 w-5" />Eventos cadastrados</CardTitle></CardHeader><CardContent className="space-y-3">{loading ? <p className="text-sm text-muted-foreground">Carregando…</p> : events.length === 0 ? <div className="py-10 text-center text-sm text-muted-foreground">Nenhum evento cadastrado. Use “Novo evento”.</div> : events.map((event) => <div key={event.id} className="flex flex-col gap-3 rounded-xl border p-4 lg:flex-row lg:items-center lg:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><strong>{event.title}</strong><Badge variant={event.status === 'published' ? 'default' : 'secondary'}>{event.status}</Badge></div><p className="mt-1 text-sm text-muted-foreground">{new Date(event.starts_at).toLocaleString('pt-BR')} · {event.capacity} vagas · {brl(event.price_cents)}</p></div><div className="flex flex-wrap gap-2"><Button size="sm" variant="outline" onClick={() => edit(event)}><Pencil className="mr-1 h-4 w-4" />Editar</Button><Button size="sm" variant="outline" onClick={() => void showRegistrations(event)}><Users className="mr-1 h-4 w-4" />Inscrições</Button>{event.status !== 'published' && <Button size="sm" onClick={() => void changeStatus(event, 'published')}><CheckCircle2 className="mr-1 h-4 w-4" />Publicar</Button>}{event.status === 'published' && <Button size="sm" variant="destructive" onClick={() => void changeStatus(event, 'cancelled')}><XCircle className="mr-1 h-4 w-4" />Cancelar</Button>}<Button asChild size="sm" variant="ghost"><a href="/eventos" target="_blank" rel="noreferrer"><Eye className="mr-1 h-4 w-4" />Ver público</a></Button></div></div>)}</CardContent></Card>
  </div>;
}
