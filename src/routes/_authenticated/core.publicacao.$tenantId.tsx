...
function PublicacaoDetail() {
  ...
  async function runApprove() {
    ...
    try {
      ...
      await qc.invalidateQueries({ queryKey: ["tenant-publications"] });
      toast.success("Publicação aprovada. Execute o pipeline canônico GitHub → VPS para promover o snapshot.");
    } catch (e: any) {
  ...
  return (
    <div className="p-6 space-y-6 max-w-6xl mx-auto">
      ...
      <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
        ...
        <PublicationValidationCard title="2. DNS (A/CNAME + validação Cloudflare)" result={detail.dns} />
      ...
      <Card className="p-4 text-xs bg-amber-500/5 border-amber-500/30 space-y-1">
        ...
        <p className="text-muted-foreground">
          ...
          O deploy físico é executado pelo pipeline canônico do GitHub para a VPS, promovendo uma release imutável
          pelo ponteiro <code>/current</code> e pelo serviço <code>impulsionando-core</code>. Em caso de falha,
          o pipeline deve restaurar o snapshot anterior e validar novamente os domínios.
        </p>
      </Card>
    </div>
  );
