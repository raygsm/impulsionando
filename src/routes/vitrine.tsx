      ...
      <main className="flex-1">
        <section className="border-b bg-gradient-to-br from-primary/10 via-background to-background">
          <div className="mx-auto max-w-6xl px-4 py-14 text-center sm:px-6 sm:py-20">
            ...
            <p className="mx-auto mt-4 max-w-3xl text-base leading-relaxed text-muted-foreground sm:text-lg">A lista vem diretamente do cadastro canônico do Core. Novos clientes e subdomínios entram nesta vitrine sem manter uma segunda lista manual.</p>
        ...
        <section className="mx-auto max-w-7xl px-4 pb-20 sm:px-6 lg:px-8">
          {companiesQuery.isLoading ? <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{Array.from({length:6}).map((_,i)=><Card key={i} className="h-56 animate-pulse bg-muted/40" />)}</div> : rows.length === 0 ? <Card className="p-10 text-center"><Building2 className="mx-auto h-8 w-8 text-muted-foreground" /><h2 className="mt-3 font-semibold">Nenhum cliente encontrado</h2><p className="mt-1 text-sm text-muted-foreground">Altere a busca ou o segmento.</p></Card> : <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{rows.map((c) => <Card key={c.id} className="flex flex-col p-6 transition-shadow hover:shadow-elegant"><div className="flex items-start justify-between gap-3"><div className="grid h-11 w-11 place-items-center rounded-xl bg-primary/10 text-primary"><Building2 className="h-5 w-5" /></div><Badge variant="outline">{c.segment || "serviços"}</Badge></div><h2 className="mt-4 text-xl font-semibold">{c.trade_name || c.name}</h2><p className="mt-2 flex-1 text-sm leading-relaxed text-muted-foreground">{c.description || c.tagline || "Operação conectada ao Core Impulsionando."}</p><div className="mt-5 flex gap-2">{c.public_slug ? <Button asChild size="sm" variant="outline" className="flex-1"><Link to="/vitrine/$slug" params={{slug:c.public_slug}}>Perfil</Link></Button> : null}{c.website ? <Button asChild size="sm" className="flex-1"><a href={c.website} target="_blank" rel="noreferrer"><Globe className="mr-2 h-3.5 w-3.5" />Abrir site</a></Button> : null}</div></Card>)}</div>}
        </section>
      </main>
      <PublicFooter />
    </div>
