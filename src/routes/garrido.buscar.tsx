...
export const Route = createFileRoute("/garrido/buscar")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Filtre imóveis por finalidade, categoria, cidade, bairro, quartos, preço e ordenação. Encontre o imóvel ideal com a Garrido." },
      { property: "og:title", content: "Buscar imóveis — Imobiliária Garrido" },
      { property: "og:description", content: "Compra, aluguel, temporada, lançamentos, alto padrão, comercial e rural." },
    ],
  }),
...
function BuscarPage() {
  ...
  const resultados = useMemo(() => {
    ...
    if (precoMax > 0) list = list.filter((i) => (i.precoVenda ?? i.precoAluguel ?? Infinity) <= precoMax);

    switch (ordem) {
      case "menor-preco":
        list.sort((a, b) => (a.precoVenda ?? a.precoAluguel ?? Infinity) - (b.precoVenda ?? b.precoAluguel ?? Infinity));
        break;
      case "maior-preco":
    ...
    return list;
  }, [q, finalidade, categoria, cidade, bairro, quartos, precoMax, ordem]);

  const limparFiltros = () => {
    setQ(""); setFinalidade(""); setCategoria(""); setCidade(""); setBairro(""); setQuartos(0); setPrecoMax(0); setOrdem("recentes");
  };
  ...
  return (
    <section className="py-8 md:py-10">
      <div className="max-w-7xl mx-auto px-4">
        {/* Breadcrumb */}
        <nav aria-label="Trilha" className="text-xs text-slate-500 mb-4">
          <Link to="/garrido" className="hover:underline">Início</Link> <span className="mx-1">›</span> <span className="text-[color:var(--garrido-ink)] font-semibold">Buscar imóveis</span>
        </nav>

        <h1 className="font-serif text-2xl md:text-3xl font-bold text-[color:var(--garrido-ink)]">Buscar imóveis</h1>
        <p className="text-sm text-slate-500 mt-1">Use os filtros para encontrar o imóvel ideal.</p>
        ...
        <div className="mt-6 bg-white rounded-xl p-3 shadow-sm border border-black/5 flex flex-col md:flex-row gap-2">
          ...
          <button
            ...
            className="md:hidden rounded-lg px-4 py-2.5 border border-black/10 font-semibold inline-flex items-center justify-center gap-2"
          >
        ...
        <div className="mt-6 grid lg:grid-cols-[280px_1fr] gap-6">
          {/* Filtros */}
          <aside className={`${mobileFiltersOpen ? "fixed inset-0 z-50 bg-black/50 lg:static lg:bg-transparent" : "hidden lg:block"}`}>
            <div className={`${mobileFiltersOpen ? "absolute right-0 top-0 h-full w-full max-w-xs bg-white p-4 overflow-y-auto lg:static lg:max-w-none lg:p-0" : ""}`}>
              <div className="lg:sticky lg:top-24 bg-white rounded-xl border border-black/5 shadow-sm p-4 space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <button className="lg:hidden p-1" onClick={() => setMobileFiltersOpen(false)} aria-label="Fechar filtros">
                    <X className="h-4 w-4" />
                  </button>
                </div>
                ...
                <Field label={`Mín. de quartos: ${quartos || "qualquer"}`}>
                  <input
                ...
                <button
                  ...
                  className="w-full text-xs text-[color:var(--garrido-ink)] font-semibold py-2 border border-black/10 rounded-md hover:bg-[color:var(--garrido-cream)]"
                >
                  Limpar filtros
                </button>
              </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              <div className="text-sm text-slate-600" aria-live="polite">
                <span className="font-semibold text-[color:var(--garrido-ink)]">{resultados.length}</span> {resultados.length === 1 ? "imóvel encontrado" : "imóveis encontrados"}
              </div>
            ...
            {resultados.length === 0 ? (
              <div className="text-center py-20 bg-white rounded-xl border border-dashed border-black/10">
                ...
                <div className="font-semibold text-[color:var(--garrido-ink)]">Nenhum imóvel encontrado</div>
                <p className="text-sm text-slate-500 mt-1">Tente ampliar os filtros ou fale com um corretor.</p>
                <Link to="/garrido/contato" className="mt-4 inline-block rounded-md bg-[color:var(--garrido-ink)] text-white text-sm font-semibold px-4 py-2">
                  Falar com corretor
                </Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                {resultados.map((i) => <Card key={i.slug} i={i} />)}
              </div>
            )}
          </div>
        </div>
      </div>
      <style>{`.input{width:100%;border:1px solid rgba(0,0,0,.1);border-radius:8px;padding:.5rem .625rem;font-size:.875rem;background:#fff;}`}</style>
    </section>
  );
}
...
function Card({ i }: { i: Imovel }) {
  ...
  return (
    <Link to="/garrido/imovel/$slug" params={{ slug: i.slug }} className="group bg-white rounded-xl overflow-hidden border border-black/5 hover:shadow-xl hover:-translate-y-0.5 transition">
      <div className="relative aspect-[4/3] bg-slate-100 overflow-hidden">
        <img src={i.fotos[0]} alt={i.titulo} loading="lazy" className="w-full h-full object-cover group-hover:scale-105 transition duration-500" />
        <span className="absolute top-3 left-3 rounded-md bg-white/95 text-[color:var(--garrido-ink)] text-[10px] uppercase tracking-wider px-2 py-1 font-bold shadow-sm">
          {i.finalidade[0] === "venda" ? "Venda" : i.finalidade[0] === "aluguel" ? "Aluguel" : i.finalidade[0] === "temporada" ? "Temporada" : "Lançamento"}
        </span>
      </div>
      <div className="p-4">
