...
| **R-12** | **PROPOSED models mistaken for acceptance.** Detailed product/database plans look executable. | Accidental schema/cardinality/commercial authority | Nest-first is plan direction only; product/DB decisions, ADRs and G0–G3 remain explicit | G0/G3 |
...
| **R-17** | Draft frontend treated as landed | Work targets unavailable runtime | ADR-002 governs unless ADR-009 is accepted and landed; Nest contracts stay framework-independent |
| **R-18** | Canonical DB big bang over 577 live tables | Data loss/parallel writers | DB1 decision and capability-sized DB2–DB9; UNKNOWN blocks |
| **R-19** | Conversion milestones double-counted | False Growth metrics/effects | Accept P-DB-06 before metric/write acceptance |
| **R-20** | First CRM slice widens AI scope | Cross-tenant context/effects | One governed runtime; agent READ only; P-DB-09 pending; no standing raw portfolio context |
| **R-21** | Original S/P/A, product D and local F/C labels become competing plans | Incompatible execution | Phase 8 board wins; local labels remain subordinate checkpoints |
