...
Recorded: **2026-09-04T12:10Z** (CSI staging image `…64411dbebe…-csi7b` live after hung-load retry; Host `stg.csi…`; prod-shaped Host-header unchanged; no prod DNS flip). Update when facts change. **No secrets.**
...
| CSI core SSR (Phase 7B staging) | Swarm **`reengineering-csi-core`** · image `ghcr.io/raygsm/impulsionando-csi-core:64411dbebe72218f6aded32b5442513e12e8730f-csi7b` (linux/amd64 **local-load** 2026-09-04; scp+`docker load` after hung pipe kill) · Traefik Host **`stg.csi.impulsionando.com.br`** (`stg` first) → `:3000` · gitSha bake `64411dbebe…` · in-container Host `/`+`/csi` CSI Invest HTML **200**; Traefik gate → **401** without vault auth; public DNS A for `stg.csi` **pending** · workers OFF · staging Supabase baked at build |
