...
Recorded: **2026-09-04T16:01Z** (CSI staging image `…f889a87e…-csi7b` live with `stg.<tenant>` host recognition; Host `stg.csi…`; prod-shaped Host-header unchanged; no prod DNS flip). Update when facts change. **No secrets.**
...
| CSI core SSR (Phase 7B staging) | Swarm **`reengineering-csi-core`** · image `ghcr.io/raygsm/impulsionando-csi-core:f889a87e2b4b878ef4b87e6b49f457ead6894fc3-csi7b` (linux/amd64 **local-load** 2026-09-04T16:01Z; scp+`docker load`) · Traefik Host **`stg.csi.impulsionando.com.br`** (`stg` first) → `:3000` · `/healthz` gitSha `f889a87e2b4b878ef4b87e6b49f457ead6894fc3` · Host `/`+`/csi` CSI Invest HTML **200**; **“Domínio não reconhecido” ABSENT** · Traefik gate ON · public DNS A for `stg.csi` **pending** · workers OFF · staging Supabase baked at build · rollback tag `…64411dbebe…-csi7b` |
