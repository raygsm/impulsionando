...
Atualizado em: 2026-09-04T22:50Z (Phase 6 **CLOSED**; Phase 7 **IN PROGRESS** — **7A PASS**; **7B = CSI**; prod DNS **BLOCKED**; **7F PARKED**. Phase 8 **PLANNING** — G0 pending; product model **PROPOSED**; Impeccable **design corpus LANDED as docs** (no UI rewrite); canonical DB drafts **PROPOSED** not applied; FPS **PLANNED — BLOCKED**; ADR-009 Proposed Next shell on program)
...
| Product + design corpus (Impeccable) | **LANDED (docs)** on `reengineering/program` — App de balcão, IA, UX specs, tokens, a11y, PT-BR copy, HTML artifacts — [`06-autonomous-marketing-platform/design/README.md`](06-autonomous-marketing-platform/design/README.md) · root [`PRODUCT.md`](../../PRODUCT.md) / [`DESIGN.md`](../../DESIGN.md). **No further UI rewrite** from this track until an explicit restyle task (keep shell; change color/type/components only). Logo mark = `OPEN` (wordmark). |
...
| Nest FPS work-order paper | **PAPER** — overlay docs in [`phase-8/first-product-slice/`](04-migration/phase-8/first-product-slice/) (ALLOW-DENY, CONTRACTS, NEST-WORK-ORDER, STAKEHOLDER-DELTA, …) · **no code** |
...
| Phase 8 technical target | Modular Nest API is product/domain authority; `apps/app-web` is presentation/thin BFF. ADR-002 currently keeps TanStack; **ADR-009 Proposed** + Next.js shell landed on `reengineering/program` from `#151` (not an accepted frontend gate). The accepted frontend ADR at implementation time governs. |
